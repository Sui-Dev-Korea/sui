...
`TEXT` 타입은 transaction digest를 쉽게 읽을 수 있고 외부 도구에서 바로 사용할 수 있도록 하기 위해 사용된다. digest는 `Base58`로 인코딩되며 PostgreSQL은 이 형식으로 `BYTEA` 데이터를 기본적으로 표시할 수 없기 때문에 이를 `TEXT`로 저장하면 쿼리에서 digest를 복사해 [SuiScan](https://suiscan.xyz/testnet/home)과 같은 explorer에 붙여넣어 바로 사용할 수 있다.

그러나 운영 환경에서는 `BYTEA`를 강력히 권장한다. 이는 raw byte 표현을 저장함으로써 문자열보다 더 압축된 형태로 공간을 사용하고 비교 역시 훨씬 빠르게 수행할 수 있어 우수한 storage 및 쿼리 효율을 제공하기 때문이다. 더 많은 정보를 확인하려면 CYBERTEC 웹사이트의 [Binary data performance in PostgreSQL](https://www.cybertec-postgresql.com/en/binary-data-performance-in-postgresql/)을 참고한다.
