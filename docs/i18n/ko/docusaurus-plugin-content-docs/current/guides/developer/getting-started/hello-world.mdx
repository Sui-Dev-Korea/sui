...
Sui에서 가장 기본적인 storage 단위는 [object](/guides/developer/objects/object-model.mdx)이다.
...
[Move](/concepts/sui-move-concepts.mdx)는 Sui가 smart contract를 생성하는 데 사용하는 프로그래밍 언어이다.
...
- [x] [Sui 설치](/guides/developer/getting-started/sui-install.mdx).

- [x] [Sui 클라이언트 설정](/guides/developer/getting-started/configure-sui-client.mdx).

- [x] [Sui 주소 만들기](/guides/developer/getting-started/get-address.mdx).

- [x] [Faucet에서 SUI 받기](/guides/developer/getting-started/get-coins.mdx).
...
`testnet`을 반환하지 않으면 계속하기 전에 [client configuration instructions](/guides/developer/getting-started/configure-sui-client.mdx)를 따른다.
...
잔액이 없다면 [SUI faucet instructions](/guides/developer/getting-started/get-coins.mdx)를 따른다.
...
나중에 [Move 패키지에 프런트엔드 연결](/guides/developer/getting-started/app-frontends.mdx) 가이드에서 사용하므로 두 값을 모두 기록해 둔다.
...
여러 transaction command를 함께 batch하고 싶다면 [프로그래머블 트랜잭션 블록 (PTB)](/guides/developer/transactions/ptbs/prog-txn-blocks)를 사용할 수 있다.
