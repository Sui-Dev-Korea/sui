---
title: 대체 불가능 토큰 만들기
slug: /guides/developer/nft
description: Sui에서는 모든 것이 object이다. 또한 object는 고유하고 대체 불가능하며 owned이므로 모든 것은 대체 불가능 토큰 (NFT)이다.
...
이 예제에서는 누구나 `mint_to_sender` 함수를 호출하여 NFT를 민트할 수 있다. 이름에서 알 수 있듯이 이 함수는 새로운 `TestnetNFT`를 생성하고 이를 호출을 수행한 address로 전송한다.
