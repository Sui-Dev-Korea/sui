...
이 가이드는 커스텀 인덱서(custom indexer)를 구축하는 실용적인 예시를 제공한다. indexer framework의 개념적 개요는 [커스텀 인덱싱 프레임워크](/concepts/data-access/custom-indexers) 및 [인덱서 파이프라인 아키텍처](/concepts/data-access/pipeline-architecture)를 참고한다.
