...
- 이를 수정하는 모든 transaction마다 단조 증가하는 8바이트 부호 없는 정수 버전 ([객체 및 패키지 버전 관리](./versioning.mdx) 참조).
- 이 object를 출력으로 포함한 마지막 transaction을 나타내는 32바이트 transaction digest.
- 이 object에 접근할 수 있는 방법을 나타내는 32바이트 소유자 필드. 자세한 내용은 [객체 소유권 유형](./object-ownership/index.mdx)을 참조한다.
