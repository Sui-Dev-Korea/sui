...
Sui smart contracts는 Move 모듈의 컬렉션으로 구성된 변경 불가능한 패키지 object이다. 패키지가 변경 불가능하기 때문에 transaction은 전체 합의 없이도 smart contract에 안전하게 접근할 수 있다(fastpath transactions). 만약 누군가 이러한 패키지를 변경할 수 있다면, 이는 [shared objects](/guides/developer/objects/object-ownership/shared.mdx#shared)가 되어 transaction을 완료하기 전에 전체 합의가 필요하다.
...
패키지가 `v1.29.0` 이전의 Sui 버전으로 게시되거나 업그레이드된 경우, [the guide for adopting automated address management](automated-address-management.mdx#adopting-automated-address-management-for-published-packages)를 따를 수 있다. 또는 추가 단계에 대해서는 위의 `Manual Addresses` 탭을 참조한다.
...
패키지가 [adopted automated address management](automated-address-management.mdx#adopting-automated-address-management-for-published-packages)을 아직 하지 않았다면 다음과 같은 수동 단계를 수행해야 한다.
...
패키지가 `v1.29.0` 이전의 Sui 버전으로 게시되거나 업그레이드된 경우, [자동 주소 관리 채택을 위한 가이드](automated-address-management.mdx#adopting-automated-address-management-for-published-packages)를 따를 수 있다. 또는 추가 단계에 대해서는 위의 `Manual Addresses` 탭을 참조한다.
