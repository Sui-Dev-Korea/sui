...
- [upgrading](./upgrade.mdx#upgrade-requirements) 시, 패키지의 `UpgradeCap` ID가 필요하다. Automated address management는 `UpgradeCap`을 추적하지 않는다.
- [upgrading](./upgrade.mdx#example) 시, `Move.toml`에서 패키지의 `[addresses]` 값을 `0x0`으로 설정하고, 업그레이드 후 `ORIGINAL-ADDRESS`로 복원한다.
