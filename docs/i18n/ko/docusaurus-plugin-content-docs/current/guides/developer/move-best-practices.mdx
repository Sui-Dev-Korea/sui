...
이러한 이유로 `Move.lock` 파일은 항상 패키지에 포함되어야 한다 (`.gitignore` 파일에 추가하지 말아야 한다). 매니페스트 파일의 이전 `published-at` 필드 대신 [자동 주소 관리](/guides/developer/packages/automated-address-management.mdx)를 사용한다.
