...
- **Shared objects:** 이 가이드는 [shared objects](/guides/developer/objects/object-ownership/shared.mdx)를 사용하는 방법을 설명하며, 이 예시에서는 전역에서 접근 가능한 `HouseData` object를 생성하는 데 사용한다.
- **One-time witnesses:** 이 가이드는 [one-time witnesses](/concepts/sui-move-concepts.mdx)를 사용하는 방법을 설명하며, 이를 통해 `HouseData` object의 단일 인스턴스만 존재하도록 보장한다.
...
- **Address-owned objects:** 이 가이드는 필요할 때 [address-owned objects](/guides/developer/objects/object-ownership/address-owned)를 사용하는 방법을 설명한다.
- **Events:** 이 가이드는 contract에서 이벤트를 발생시키는 방법을 설명하며, 이를 통해 온체인 활동을 추적할 수 있다. 이벤트에 대해 더 알아보려면 Sui에서 이벤트를 실제로 사용하는 방법을 다룬 [이벤트 사용하기](/guides/developer/accessing-data/using-events.mdx)와 이벤트 구조 및 Move에서 이벤트를 발생시키는 방법을 설명하는 [Events in The Move Book](https://move-book.com/programmability/events.html)을 참조한다.
- **스토리지 리베이트:** 이 가이드는 [storage fee rebates](/concepts/tokenomics/tokenomics-overview.mdx)에 관한 모범 사례를 보여준다.
...
첫 번째 단계는 Move module을 저장할 [set up a Move package](/guides/developer/getting-started/hello-world.mdx)이다.
...
게임 데이터를 어딘가에 저장해야 하며, 이 module에서는 모든 house 데이터를 위한 [shared object](/guides/developer/objects/object-ownership/shared)를 생성한다.
...
이것은 1인용 게임이므로 [shared object](/guides/developer/objects/object-ownership/shared.mdx)가 아니라 [address-owned object](/guides/developer/objects/object-ownership/address-owned) 를 만든다.
...
- 먼저 함수는 `Game` object가 존재하는지 확인하고, 게임이 끝난 뒤에는 metadata가 더 이상 필요하지 않으므로 이를 삭제한다. 불필요한 storage를 해제하는 것은 권장될 뿐 아니라 [storage fee rebates](/concepts/tokenomics/tokenomics-overview.mdx)를 통해 장려된다.
...
이 UI는 게임의 [Single Player smart contract](/guides/developer/app-examples/coin-flip.mdx) 변형과 상호작용한다.
...
Sui에서 [프로그래머블 트랜잭션 블록 (PTB)](/guides/developer/transactions/ptbs/prog-txn-blocks)을 사용하려면 `Transaction`을 생성한다.
...
dApp에서 이벤트를 다루는 방법에 대해 더 알아보려면 [이벤트 사용하기](/guides/developer/accessing-data/using-events.mdx)를 참조한다.
