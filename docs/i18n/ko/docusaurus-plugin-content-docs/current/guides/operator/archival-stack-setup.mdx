...
스택에 대한 자세한 내용은 [아카이브 스토어 및 서비스](/concepts/data-access/archival-store.mdx)를 참조하라.
...
- gRPC가 활성화된 풀 노드. 설정 방법은 [Sui 풀 노드 구성](/guides/operator/sui-full-node.mdx)을 참조하라. 초기 백필 이후에는 더 낮은 지연 시간을 위해 bucket을 폴링하는 대신 풀 노드에서 checkpoint를 스트리밍할 수 있다. [Steady-state operation](#steady-state-operation)을 참조하라.
