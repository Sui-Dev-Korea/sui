...
스택에 대한 자세한 내용은 [GraphQL 및 General-purpose Indexer](/concepts/data-access/custom-indexers.mdx)를 참조하라.
...
| `<REMOTE_STORE_URL>` | indexing에 사용할 checkpoint bucket URL이다. 가능한 여러 [data sources](/guides/developer/accessing-data/custom-indexing-framework#data-sources) 중 하나를 사용한다. |
...
| CLI parameter               | 설명 |
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| `<AZURE_BUCKET>` \*         | [managed snapshots](https://docs.sui.io/guides/operator/snapshots#mysten-labs-managed-snapshots)를 담고 있는 Azure bucket의 이름 또는 URL이다. |
| `<DATABASE_PATH>`           | RocksDB database 경로이다. |
| `<GCS_ACCOUNT>` \*          | [managed snapshots](https://docs.sui.io/guides/operator/snapshots#mysten-labs-managed-snapshots)를 담고 있는 GCS bucket의 이름 또는 URL이다. |
...
| `<REMOTE_STORE_URL>`        | indexing에 사용할 checkpoint bucket URL이다. 가능한 여러 [data sources](/guides/developer/accessing-data/custom-indexing-framework#data-sources) 중 하나를 사용한다. |
| `<S3_BUCKET>` \*            | [managed snapshots](https://docs.sui.io/guides/operator/snapshots#mysten-labs-managed-snapshots)를 담고 있는 AWS S3 bucket의 이름 또는 URL이다. |
...
GraphQL RPC server는 General-purpose Indexer의 database(Postgres), Consistent Store, 그리고 [archival service](/guides/developer/accessing-data/archival-store.mdx)에서 데이터를 읽는다.
