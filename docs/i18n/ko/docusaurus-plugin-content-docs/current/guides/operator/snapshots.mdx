...
포멀 스냅샷은 과거 data lookup을 수행하는 RPC node를 운영하는 경우에는 적합하지 않다. node data management에 대한 자세한 내용은 [데이터 관리](./data-management.mdx)를 참고한다.
...
자세한 내용은 [Sui 설치](/guides/developer/getting-started/sui-install.mdx)를 참고한다.
