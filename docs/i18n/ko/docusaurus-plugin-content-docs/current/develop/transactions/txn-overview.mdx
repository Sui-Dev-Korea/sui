...
- **[프로그래머블 트랜잭션 블록 (PTB)](/develop/transactions/ptbs/prog-txn-blocks):** smart contract 배포와 token 전송 같은 일상적인 네트워크 활동을 가능하게 한다. 누구나 PTB를 생성하고 제출할 수 있다.
...
- **Sender address:** transaction을 보내는 사용자의 [address](/getting-started/onboarding/get-address)이다.

- **Gas input:** transaction 실행과 저장 비용을 지불하는 데 사용하는 object를 가리키는 reference이다. sender address는 이 object를 소유해야 하며 이 object의 타입은 `sui::coin::Coin<SUI>`여야 한다. 또는 gas payment가 빈 list일 수 있으며, 이 경우 transaction은 sender의 address balance에서 gas를 지불한다.
