---
title: 다중 서명 인증
description: multisig 트랜잭션을 생성한 다음 Sui CLI를 사용해 로컬 network에 제출하는 방법에 대한 가이드이다.
...
threshold signature와 비교하면 multisig account는 일반적으로 복잡한 multi-party computation(MPC) account setup ceremony와 관련 software, threshold crypto provider에 대한 dependency 없이 더 유연하고 구현 및 사용이 간단하다. 또한 key scheme을 섞어 쓰고 각 key에 서로 다른 weight를 설정할 수 있다는 점(이는 threshold cryptography에서는 복잡함) 외에도, multisig account는 참여 party와 observer가 각 트랜잭션에 누가 서명했는지 볼 수 있으므로 설계상 accountable하고 transparent하다. 반면 threshold signature는 threshold policy를 숨기는 이점이 있지만 single signature payload를 만들기 때문에 single-key account와 구별하기 어렵다.
...
트랜잭션은 객체 transfer, 패키지 publish 또는 upgrade, SUI payment 등이 될 수 있다.
...
객체를 받으려면 작업 중인 network에 이미 존재하는 address가 하나 필요하다.
...
다중 서명을 보여주기 위해 이 문서는 Sui CLI를 사용해 다중 서명 트랜잭션을 설정하고 실행하는 과정을 안내한다.

### 다른 scheme으로 address 생성

먼저 나중에 instruction에서 수행할 트랜잭션의 signer 역할을 할 address 세 개를 만든다.
...
다중 서명을 사용해 트랜잭션에 서명하려면 `sui keytool multi-sig-address`를 사용해 다중 서명 주소를 만들어야 한다.
...
마지막 address는 `3` 값과 같은 `threshold`의 weight를 가지므로 그 소유자만으로도 유효한 signature를 만들 수 있다.
...
`sui client objects $MULTISIG` command를 사용하면 새로 만든 다중 서명 주소에 객체가 없다는 것을 볼 수 있다.
이는 트랜잭션을 수행하기 전에 SUI를 받아야 함을 뜻한다.
...
이제 다중 서명 주소에서 객체를 transfer할 차례이다.
단순화를 위해 이 예시는 다중 서명 주소가 소유한 코인 중 하나를 transfer 객체로 사용한다.
address의 코인 하나에 대한 객체 ID를 복사하고 이를 사용해 shell variable 값을 설정한다.
...
`--serialize-unsigned-transaction` flag는 Base64로 인코딩된 트랜잭션 bytes를 출력한다.
...
### 두 public key로 트랜잭션 sign

`sui keytool sign` command를 사용해 앞서 만든 address 중 두 개로 트랜잭션에 서명한다.
...
`sui client execute-signed-tx`를 사용해 다중 서명 트랜잭션을 실행한다.
...
성공하면 console은 트랜잭션 detail을 응답한다.
