---
title: 트랜잭션 서명 및 전송
description: Sui의 cryptographic key, address, signature를 이해한다. Sui는 여러 cryptography algorithm과 primitive를 지원하며 이들 사이를 빠르게 전환할 수 있다.
...
Sui는 사용자의 key 관리를 돕기 위해 cryptocurrency 업계에서 널리 받아들여진 wallet specification인 [BIP-32](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki)와 그 변형인 [SLIP-0010](https://github.com/satoshilabs/slips/blob/master/slip-0010), [BIP-44](https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki), [BIP-39](https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki)를 따른다. Sui는 signed transaction에 대해 pure Ed25519, ECDSA Secp256k1, ECDSA Secp256r1, multisig를 지원한다.

### key derivation scheme

Ed25519(EdDSA) signing scheme을 지원하는 wallet을 관리할 때 Sui는 [SLIP-0010](https://github.com/satoshilabs/slips/blob/master/slip-0010)을 따른다. SLIP-0010은 wallet이 hardened key path를 사용해 parent private key에서 child private key를 항상 derive하도록 강제한다.
...
예를 들어 BIP-49와 BIP-84는 Bitcoin의 script type을 식별하는 데 사용된다. Sui는 ECDSA Secp256k1을 나타내기 위해 `54`를 선택했다. `54` 아래에는 기존 BIP가 없어서 Bitcoin standard와 혼동될 가능성을 피할 수 있기 때문이다.

`coin_type` 값은 다른 cryptocurrency와 함께 repository에서 관리된다. 두 signature scheme 모두 Sui의 등록된 `coin_type`인 `784`를 사용한다.

`account` level은 user account를 논리적으로 분리하고 특정 account category를 만드는 데 사용된다.

Account-based currency는 처음 3개 level만 정의하는 반면, UTXO-based currency는 `change`와 `address` level 정의를 추가한다. Sui의 object-oriented data model은 UTXO 기반도 account 기반도 아니며 두 방식을 결합하므로, 최대한의 호환성을 위해 5개 level을 모두 사용한다.
...
32-byte Sui address를 derive하기 위해 Sui는 signature scheme flag 1-byte와 public key byte를 연결한 값을 [BLAKE2b](https://www.blake2.net/)(256-bit output) hashing function으로 hash한다. Sui address는 현재 pure Ed25519, Secp256k1, Secp256r1, [multisig](/develop/transactions/transaction-auth/multisig)를 지원하며, 각각 대응하는 flag byte는 0x00, 0x01, 0x02, 0x03이다.

## 서명

사용자가 signed transaction을 제출하면 serialized signature와 serialized transaction data가 제출된다. serialized transaction data는 `TransactionData` struct를 [Binary Canonical Serialization](https://crates.io/crates/bcs)으로 serialize한 byte이고, serialized signature는 `flag || sig || pk` byte를 연결한 값으로 정의된다.
...
signature는 transaction data의 intent message hash에 commit해야 한다. 이 hash는 BCS serialized transaction data 앞에 3-byte intent를 붙여 구성할 수 있다. intent가 무엇인지와 intent message를 구성하는 방법을 자세히 알아보려면 [Intent Signing](/develop/transactions/transaction-auth/intent-signing)을 참조한다.
...
1. ECDSA가 사용하는 internal hash는 transaction data의 SHA256 [SHA-2](https://en.wikipedia.org/wiki/SHA-2) hash여야 한다. Sui가 SHA256을 사용하는 이유는 [Apple](https://developer.apple.com/forums/thread/89619), HSM에서 지원되고 [Bitcoin](https://en.bitcoin.it/wiki/Elliptic_Curve_Digital_Signature_Algorithm)에서도 널리 채택되었기 때문이다.

1. signature는 첫 32 byte가 `r`, 두 번째 32 byte가 `s`인 `[r, s]` 형식의 64 byte 길이여야 한다.

1. `r` 값은 `0x1`부터 `0xFFFFFFFF FFFFFFFF FFFFFFFF FFFFFFFE BAAEDCE6 AF48A03B BFD25E8C D0364140`까지의 범위(포함)에 있어야 한다.

1. `s` 값은 curve order의 lower half에 있어야 한다. signature가 너무 높으면 해당 curve order에 대해 `order - s`를 사용해 [BIP-0062](https://github.com/bitcoin/bips/blob/master/bip-0062.mediawiki#low-s-values-in-signatures)에 따라 더 낮은 `s`로 변환한다. Secp256k1의 curve order는 `0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141`이다. Secp256r1의 curve order는 [Standards for Efficient Cryptography](https://secg.org/SEC2-Ver-1.0.pdf)에 정의된 `0xFFFFFFFF00000000FFFFFFFFFFFFFFFFBCE6FAADA7179E84F3B9CAC2FC632551`이다.

1. 이상적으로 signature는 [RFC6979](https://www.rfc-editor.org/rfc/rfc6979)에 따라 deterministic nonce로 생성되어야 한다.

### Ed25519 signature requirement

허용되는 pure Ed25519 signature는 다음 조건을 따라야 한다:

1. signature는 [RFC 8032](https://www.rfc-editor.org/rfc/rfc8032#section-5.1.6)에 따라 생성되어야 한다. internal hash로는 SHA-512를 사용한다.

1. signature는 [ZIP215](https://github.com/zcash/zips/blob/main/zips/zip-0215.rst)에 따라 valid해야 한다.

### 특수 signature scheme

advanced signature scheme에 대한 자세한 내용은 다음을 참조한다:

- **zkLogin**: zero-knowledge login signature의 자세한 내용은 [zkLogin](/sui-stack/zklogin-integration)을 참조한다.

- **Passkey**: passkey implementation의 자세한 내용은 [SIP-8](https://github.com/sui-foundation/sips/blob/main/sips/sip-9.md)을 참조한다.

- **Multisig**: multi-signature transaction의 자세한 내용은 [Multisig](/develop/transactions/transaction-auth/multisig)를 참조한다.

- **Offline Signing**: offline transaction signing의 구체적인 예시는 [Offline Signing](/develop/transactions/transaction-auth/offline-signing)을 참조한다.

## Authority 서명

Sui의 [validator collection](/operators/validator)은 서로 다른 목적을 위해 3개의 distinct key pair를 보유한다.
...
private key는 [consensus](/develop/sui-architecture/consensus) networking에 필요한 TLS handshake를 수행하는 데 사용된다. public key는 validator identity에 사용된다. scheme으로는 pure Ed25519가 사용된다.

더 많은 authority key tooling은 [Validator Tool](https://github.com/MystenLabs/sui/blob/f8b5ad9aaecc3c4b30a060ec5e00bdad9ba75a93/nre/validator_tool.md)을 참조한다.

### 예시

[Sui CLI tool](/getting-started/onboarding/sui-install)과 [Sui SDKs](https://sdk.mystenlabs.com/typescript)는 여러 signing scheme으로 transaction에 서명할 수 있는 유연한 interface를 제공한다.
...
더 많은 test vector는 [pure Ed25519](https://github.com/MystenLabs/sui/blob/f3b5fdd73da64a0df65fb4323471512b0f57ec4d/sdk/typescript/test/unit/cryptography/ed25519-keypair.test.ts) 또는 [ECDSA Secp256k1](https://github.com/MystenLabs/sui/blob/199f06d25ce85f0270a1a5a0396156bb2b83122c/sdk/typescript/test/unit/cryptography/secp256k1-keypair.test.ts)을 참조한다.

## 워크플로

다음 상위 수준 과정은 온체인 transaction을 구성하고 서명하고 실행하는 전체 흐름을 설명한다:

1. 여러 command를 연결한 `Transaction`을 만들어 transaction data를 구성한다. 자세한 내용은 [Programmable Transaction Block 구성하기](/develop/transactions/ptbs/building-ptb)를 참조한다.

2. SDK의 내장 gas estimation과 coin selection이 gas coin을 선택한다.

3. [signature](/develop/transactions/transaction-auth/auth-overview)를 생성하도록 transaction에 서명한다.

4. `Transaction`과 그 signature를 제출해 온체인 실행을 요청한다.
...
gas coin object가 없다면 [splitCoin](/develop/transactions/ptbs/building-ptb#gas-coin-사용) transaction을 사용해 gas coin object를 만든다.
