...
Sui에서 가스 스테이션(gas station)은 사용자 transaction을 sponsor하기 위한 프로세스를 설정하는 장소를 설명하는 개념이다. 필요한 특정 사용자 지향 기능을 지원하도록 Sui 가스 스테이션을 커스터마이즈할 수 있다. Sui 가스 스테이션의 예시 유즈 케이스는 다음과 같다:
...
가스 스테이션의 성격에 따라 악의적 행위자에 의한 스팸을 방지하기 위해 다양한 권한 규칙을 적용할 수 있다. 가능한 정책은 다음을 포함한다:
...
다음 Rust SDK 코드 예시는 앞서 설명한 각 sponsored transaction 유형을 지원하는 Sui 가스 스테이션을 구현하는 방법을 보여준다.
...
Sui에서 transaction에 대해 더 알아보려면 [트랜잭션 개요](./index)을 참조하라.
