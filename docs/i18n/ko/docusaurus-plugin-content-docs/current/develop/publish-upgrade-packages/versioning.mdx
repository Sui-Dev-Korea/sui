---
title: 패키지 versioning
sidebar_label: 버전 관리
description: versioning은 Sui network에서 패키지를 upgrade할 수 있게 한다.
...
publish 또는 upgrade마다 새 ID가 생성된다. 새로 publish된 패키지는 version 1에서 시작하며, 각 upgrade는 바로 이전 version에서 version을 1씩 증가시킨다. 객체와 달리 오래된 패키지 version은 upgrade 후에도 계속 accessible하다. 두 번 upgrade된 패키지는 store에 다음과 같이 나타날 수 있다:
...
세 version은 모두 서로 다른 ID에 있으며, `v1`가 존재한 후에도 `v3`을 포함해 계속 호출할 수 있다.
...
### 패키지 manifest version

패키지 manifest file은 `[package]` section과 `[dependencies]` 모두에 version 필드를 포함한다. `publish`와 `upgrade` command는 이를 사용하지 않으므로 user-level documentation 용도일 뿐이다. manifest version이 다른 같은 패키지의 두 publish는 완전히 별개의 패키지로 취급되며 서로의 dependency override로 사용할 수 없다.
...
이 예시는 동일한 trustless 객체 swap service를 두 방식으로 구현하여 trade-off를 보여준다. 하나는 custodian이 있는 fastpath address-소유 객체를 사용하고, 다른 하나는 consensus 공유 객체를 사용한다.
...
양 party는 객체를 lock하는 것으로 시작한다. 어느 한쪽이 진행하지 않기로 하면 unlock하고 exit한다.
...
third-party custodian은 객체를 보유하고 두 객체가 모두 도착하면 match한다. `create` 함수는 `Escrow` request를 custodian에게 보내며, offered 객체를 unlock하는 동시에 해당 객체가 lock될 때 사용된 key의 ID를 기록한다:
...
custodian이 두 객체를 모두 소유하더라도 Move에서 유효한 action은 이를 올바르게 match하거나 반환하는 것뿐이다. `swap` 함수는 key ID를 비교해 sender와 recipient가 일치하고 각 party가 상대가 제공하는 것을 원하는지 검증한다:
...
첫 번째 party는 swap하려는 객체를 lock한다:
...
두 번째 party는 locked 객체를 확인하고, 관심이 있으면 자신의 객체를 shared `Escrow` 객체로 직접 전달하는 swap request를 생성한다. 이 request는 sender(swap이 완료되지 않으면 reclaim할 수 있는 party)와 intended recipient를 기록한다:
...
intended recipient는 자신의 locked 객체를 제공하여 swap을 완료한다:
...
`Escrow` 객체는 shared이고 누구나 access할 수 있지만, Move는 original sender와 intended recipient만 이 객체와 상호작용할 수 있도록 보장한다. `swap`은 locked 객체가 요청된 것과 일치하는지 검증하고, escrow된 객체를 추출하고, wrapper를 삭제한 뒤 두 객체를 새 owner에게 전달한다.
