---
title: 사용자 정의 업그레이드 정책
description: 사용자 지정 업그레이드 정책은 단일 키 소유권 업그레이드의 보안 위험을 해결하면서 운영 중인 패키지를 업그레이드하는 데 사용된다.
keywords:
  - upgrade policies
...
이러한 단일 키 업그레이드의 보안 위험을 해결하면서도 [패키지 업그레이드](/develop/publish-upgrade-packages/upgrade)를 가능하게 하기 위해, Sui는 *커스텀 업그레이드 정책*을 제공한다. 이러한 정책은 `UpgradeCap` 액세스를 보호하고 케이스별로 업그레이드를 승인하는 `UpgradeTicket` object를 발행한다.
...
패키지를 게시할 때 기본적으로 가장 느슨한 compatible 정책을 채택한다. 트랜잭션이 성공적으로 완료되기 전에 정책을 변경할 수 있는 트랜잭션의 일부로 패키지를 게시할 수 있으며, 이를 통해 기본 정책이 아닌 원하는 정책 수준으로 패키지를 체인에 처음 사용 가능하게 만들 수 있다.
...
## upgrade 개요 {#upgrade-overview}

패키지 업그레이드는 단일 트랜잭션 내에서 종단 간으로 발생해야 하며 세 가지 명령으로 구성된다:

1. **Authorization:** 업그레이드를 진행하기 위해 `UpgradeCap`으로부터 권한을 얻어 `UpgradeTicket`을 생성한다.
2. **Execution:** `UpgradeTicket`을 소비하고 패키지 바이트코드와 이전 버전과의 호환성을 검증하며, 업그레이드된 패키지를 나타내는 온체인 객체를 생성한다. 성공 시 결과로 `UpgradeReceipt`를 반환한다.
...
패키지를 게시하면 `UpgradeCap` 객체가 생성되고 패키지를 업그레이드하면 해당 객체가 업데이트된다. 이 객체의 소유자는 다음을 수행할 권한이 있다:
...
- 업그레이드는 단일 트랜잭션의 범위 내에서만 승인될 수 있다; 승인을 증명하는 `UpgradeTicket` 을 `store`할 수 없다.
...
`UpgradeTicket`을 생성한 후에는 해당 트랜잭션 내에서 사용해야 하며(나중을 위해 저장하거나, 삭제하거나, 소각할 수 없음), 그렇지 않으면 트랜잭션이 실패한다.

### 패키지 digest {#package-digest}
...
`UpgradeReceipt`는 `Upgrade` 명령이 성공적으로 실행되었다는 증거이며, Sui가 트랜잭션에 대해 생성된 객체 집합에 새 패키지를 추가했다는 증거이다. 이것은 패밀리의 최신 패키지 ID(`package: ID`)로 `UpgradeCap` (`cap: ID`로 식별됨)을 업데이트하는 데 사용된다.

Sui가 `UpgradeReceipt`를 생성한 후에는 동일한 트랜잭션 내에서 `UpgradeCap`을 업데이트하는 데 사용해야 하며(나중을 위해 저장하거나, 삭제하거나, 소각할 수 없음), 그렇지 않으면 트랜잭션이 실패한다.

## policy 격리 {#isolating-policies}
...
이러한 모범 사례는 사용자가 값을 잠그는 순간에 패키지의 업그레이드 정책이 무엇인지 명확히 하고, 패키지 사용자가 인식하지 못하거나 새 조건을 수용하기로 선택하지 않고도 정책이 시간이 지남에 따라 더 허용적으로 변화하지 않도록 보장함으로써 **정보에 입각한 사용자 동의**와 **제한된 위험**을 지지하는 데 도움이 된다.
...
이 코드는 생성자를 포함하고 커스텀 업그레이드 정책에 대한 객체 타입을 정의한다.
...
이 함수는 일일 단위만 필요하기 때문에 `Clock`  대신 `TxContext`의 epoch 타임스탬프를 사용하며, 이는 업그레이드 트랜잭션이 합의를 필요로 하지 않는다는 것을 의미한다.
...
### upgrade policy publish {#publishing-an-upgrade-policy}
...
모범 사례에 따라, Sui Client CLI를 사용하여 `UpgradeCap`에 대해 `sui::package::make_immutable`을 호출하여 정책을 불변으로 만든다. 쉘에서 변수 `upgradecap`을 생성하고 그 값을 게시 응답의 *Object Changes* 섹션에 나열된 `UpgradeCap` 객체 ID로 설정한다. 물론, 업그레이드 capability에 대한 객체 ID는 다음 예시와 다르다.
...
### testing용 패키지 생성 {#creating-testing-package}
...
### TypeScript SDK 사용 {#using-typescript-sdk}
...
### custom policy로 패키지 publish {#publishing-custom-policy}
...
다음으로, 패키지를 게시하는 트랜잭션을 구성한다. `UpgradeCap`을 화요일에 업그레이드를 허용하는 "day of the week" 정책으로 래핑하고 새 정책을 다시 보낸다:
...
마지막으로, 해당 트랜잭션을 실행하고 그 효과를 콘솔에 표시한다. 다음 스니펫은 로컬 네트워크에 대해 예제를 실행한다고 가정한다. Devnet, Testnet 또는 Mainnet에서 실행하려면 `baseUrl` 및 `network` 파라미터를 변경한다:
...
### custom policy로 패키지 upgrade {#upgrading-custom-policy}

패키지가 게시되었으므로 새 정책을 사용하여 업그레이드를 수행하는 `upgrade.js` 스크립트를 준비할 수 있다. 이것은 패키지를 빌드하기 전까지 `publish.js` 와 동일하게 동작한다. 패키지를 빌드할 때 스크립트는 `digest`도 캡처하고, 트랜잭션은 이제 세 가지 업그레이드 명령(authorize, execute, commit)을 수행한다. `upgrade.js`의 전체 스크립트는 다음과 같다:
