...
Sui의 checkpoints는 일관된 blockchain state snapshots를 나타내는 transaction batch이다. 각 checkpoint에는 완전한 transaction details, events, object changes, execution results가 보장된 순서로 포함된다. checkpoints에 대한 자세한 내용은 [트랜잭션 생명주기](/develop/transactions/transaction-lifecycle)를 참조한다.
...
- **Remote stores**는 [`https://checkpoints.mainnet.sui.io`](https://checkpoints.mainnet.sui.io/)와 같은 공개 checkpoint repositories에 연결할 수 있지만, 이 HTTPS endpoint는 최근 30일 checkpoint만 보관하므로 fallback source로만 사용해야 한다. Mainnet에서 30일보다 오래된 backfill에는 `gs://mysten-mainnet-checkpoints-use4`를 `--remote-store-gcs`와 함께 사용한다.
...
구성 예시와 각 source를 언제 사용해야 하는지에 대한 자세한 안내는 [Checkpoint Data Sources](/develop/accessing-data/custom-indexer/indexer-data-integration#checkpoint-data-sources)를 참조한다.
...
애플리케이션 전용 데이터를 위한 자체 pipelines를 구축하려면 [커스텀 인덱서 만들기](/develop/accessing-data/custom-indexer/build)를 참조한다. 필요에 맞는 구성이 이미 준비되어 있을 수 있으므로 [data indexer operator](/references/awesome-sui#indexers--data-services)에 문의할 수도 있다.
