...
## Sequential pipeline과 concurrent pipeline 비교
...
## Watermark system 개요 {#watermark-system}
...
### Scenario 1: 기본 watermark(pruning 없음)
...
## Sequential pipeline architecture 구조 {#sequential-pipeline-architecture}
...
### Sequential pipeline component 구성 {#sequential-components}
...
### Sequential pipeline backpressure 메커니즘
...
#### Channel 기반 backpressure

Sequential pipelines는 concurrent pipelines와 동일한 bounded-channel backpressure model을 사용한다:

- **Broadcaster → Processor:** `subscriber_channel_size` slots를 가진 bounded channel. 가득 차면 send가 block되며, broadcaster의 adaptive controller는 channel의 `len / capacity`를 읽고 subscriber가 뒤처질 때 `ingest_concurrency`를 줄인다.
- **Processor → Committer:** `processor_channel_size` slots (`num_cpus / 2`가 기본값). Processor의 adaptive `fanout` controller를 구동한다.

Downstream pressure는 역방향으로 전파된다. Committer가 느려지면 processor-to-committer channel이 차고 `fanout`이 minimum으로 줄어든다. 그다음 broadcaster-to-processor channel이 차며 broadcaster가 `ingest_concurrency`를 줄인다. Committer 자체는 pending checkpoints의 unbounded in-memory buffer를 보유하므로, 순서와 무관하게 도착한 checkpoints도 contiguous prefix를 구성해 commit할 수 있다.

### Performance tuning 조정
...
- `ingestion.subscriber_channel_size`: bounded broadcaster-to-processor channel의 capacity이다. `None`이면 기본값 `max(num_cpus / 2, 4)`를 사용한다. 가끔 burst가 있는 pipeline은 자체 capacity를 늘릴 수 있지만, 값이 클수록 해당 pipeline이 shared controller에 덜 찬 것처럼 보이므로 throttling이 다른 pipeline보다 늦게 trigger된다.
...
## Concurrent pipeline architecture 구조 {#concurrent-pipeline-architecture}
...
### Concurrent pipeline component 구성 {#concurrent-components}
...
### Concurrent pipeline backpressure 메커니즘 {#concurrent-backpressure}
...
#### Fixed size를 사용하는 channel-level blocking

각 channel은 가득 차면 자동으로 block되는 fixed buffer size를 가진다:

**`Broadcaster` to `Processor`**: `subscriber_channel_size` slots → `Broadcaster`가 block되어 upstream pressure가 생긴다.

<ImportContent mode="code" source="crates/sui-indexer-alt-framework/src/pipeline/mod.rs" struct="IngestionConfig" highlight="subscriber_channel_size" />
...
1. **Backpressure cascade**:
    ...
    - `Processor` channel이 가득 차고, adaptive `fanout` controller가 processor concurrency를 minimum 쪽으로 줄인다.
    - `Broadcaster`는 subscriber channel이 차는 것을 보고 adaptive `ingest_concurrency` controller로 fetch concurrency를 minimum 쪽으로 줄인다. Channel이 capacity에 도달하면 ingest tasks가 `send`에서 block되고 channel이 비워질 때까지 checkpoint ingestion이 멈춘다.
1. **End 결과**:
...
#### Concurrent pipeline `Handler` constant 설정
...
- **`ingestion.subscriber_channel_size`:** bounded broadcaster-to-processor channel의 capacity이다. `None`이면 기본값 `max(num_cpus / 2, 4)`를 사용한다. Ingestion bursts에 대한 여유가 더 필요한 pipeline에서 값을 늘린다. 값이 클수록 해당 pipeline은 adaptive ingest controller의 throttling을 다른 pipeline보다 늦게 trigger한다.
...
#### `CommitterConfig` optimization 조정
...
- **`max_chunk_size`:** 값이 클수록 pruning은 빨라지지만 database 트랜잭션는 더 길어진다.
