---
title: 커스텀 인덱서 만들기
description: "`sui-indexer-alt-framework` 모듈을 사용해 custom indexer를 구축한다. example indexer는 Sui checkpoint에서 트랜잭션 digest를 추출해 로컬 PostgreSQL에 저장하는 sequential pipeline을 보여준다."
keywords:
  - indexer
  - custom indexer
  - indexer framework
...
이 가이드는 커스텀 인덱서(custom indexer)를 구축하는 실용적인 예시를 제공한다. indexer framework의 개념적 개요는 [커스텀 인덱싱 프레임워크](/develop/accessing-data/custom-indexer/custom-indexers) 및 [인덱서 파이프라인 아키텍처](/develop/accessing-data/custom-indexer/pipeline-architecture)를 참고한다.

완전한 커스텀 인덱서를 구축하려면 `sui-indexer-alt-framework`를 사용한다. 다음 단계에서는 Sui checkpoint에서 transaction digest를 추출하여 로컬 PostgreSQL에 저장하는 sequential pipeline을 만드는 방법을 보여준다. framework의 [source code for the framework](https://github.com/MystenLabs/sui/tree/main/crates/sui-indexer-alt-framework)는 GitHub의 Sui repo에서 찾을 수 있다.

:::tip

이 예제는 간결함과 기본 지원을 위해 [Diesel](https://diesel.rs/guides/getting-started)(널리 사용되는 Rust ORM 및 query builder)과 함께 PostgreSQL을 사용하지만, `sui-indexer-alt-framework`는 유연한 storage를 위해 설계되었다. Diesel을 사용하지 않으려면 [MongoDB](https://www.mongodb.com/), [CouchDB](https://couchdb.apache.org/) 같은 다른 database를 사용하거나 다른 database client를 활용할 수 있다. 이를 위해 framework의 `Store` 및 `Connection` trait을 구현하고 `Handler::commit()` 메서드 안에 database write logic을 직접 정의한다.
...
- 트랜잭션 데이터를 추출한다: 각 checkpoint에서 트랜잭션 digest를 끌어온다.
...
- `clap`: 설정을 위한 command-line 인자 parsing을 제공한다.
...
코드를 작성하기 전에 이전 단계에서 로컬 PostgreSQL 데이터베이스를 설정했는지 반드시 확인해야 하며, 이는 indexer가 추출된 트랜잭션 데이터를 저장하는 데 필요하다.
...
먼저 데이터베이스 migration을 구성하기 위해 (`diesel.toml`과 동일한 폴더에) `cargo.toml` 파일을 생성한다.
...
데이터베이스 URL을 `diesel setup` 인수로 전달하여 필요한 디렉터리 구조를 생성하려면 `--database-url` 명령을 사용한다.
...
이 예제에서는 `TEXT`에 대해 `tx_digest` 데이터 타입을 사용하지만, 운영 환경의 indexer에서는 `BYTEA` 데이터 타입을 사용하는 것이 모범 사례이다.
...
`up.sql`을 저장한 다음 편집을 위해 `down.sql`을 연다. 파일 내용을 다음 코드로 교체하고 저장한다:
...
`handlers.rs` 디렉터리에 `src` 파일을 생성한다.
...
그 결과 데이터는 `Handler::commit`으로 전달된다.
...
`Handler` 코드 뒤에 `Processor` 로직을 추가한다.
...
Testnet에 대해 indexer를 실행하려면 `cargo run` 명령을 사용한다. 일반 운영에서는 full node gRPC를 primary source로 사용하고, remote checkpoint store는 fallback 또는 backfill 용도로만 사용하는 것을 권장한다. 자세한 내용은 [Checkpoint Data Sources](/develop/accessing-data/custom-indexer/indexer-data-integration#checkpoint-data-sources)를 참고한다:
...
공개 HTTPS checkpoint endpoint는 가장 최근 30일의 checkpoint만 보관한다. 30일보다 오래된 Mainnet backfill에는 `gs://mysten-mainnet-checkpoints-use4`를 통해 `--remote-store-gcs mysten-mainnet-checkpoints-use4`를 사용한다. 이 bucket은 Requester Pays가 활성화되어 있으며 `--remote-store-gcs` 대신 indexing framework의 `--remote-store-url` 옵션으로 접근해야 한다. [Use Requester Pays](https://cloud.google.com/storage/docs/using-requester-pays)와 [Running a Remote Store](/operators/data-management/remote-store-setup)를 참고한다.
