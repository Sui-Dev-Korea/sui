...
인덱싱 프레임워크는 서로 다른 사용 사례의 성능을 최적화하기 위한 여러 수준의 구성을 제공한다. 이 섹션은 기본 구성 옵션을 다루며, 더 복잡한 pipeline별 튜닝은 [인덱서 파이프라인 아키텍처](/develop/accessing-data/custom-indexer/pipeline-architecture)에서 다룬다.
...
let ingestion_config = IngestionConfig {
    ...
    // the slowest subscriber's channel fill fraction.
...
- `ingest_concurrency`: 높은 처리량 시나리오에서는 늘리고 메모리 사용량을 줄이려면 낮춘다.
- `ConcurrencyConfig::Fixed`: 동시에 실행되는 checkpoint fetch 수를 제어한다. 기본적으로 이는 adaptive이며 1에서 시작해 downstream broadcast channel이 얼마나 차 있는지에 따라 최대 500까지 확장된다. 컨트롤러는 진동을 피하기 위해 dead band(채움률 60%–85%)를 사용하며 channel이 빠르게 비워질 때는 concurrency를 늘리고 적체될 때는 줄인다. 정적 한도를 위해 `dead_band`로 재정의하거나 adaptive bounds를 커스터마이즈할 수 있다. Adaptive controller는 채움 비율 임곗값을 재정의하기 위한 `{Sequential,Concurrent}Config::ingestion` parameter도 노출하지만 기본값은 대부분의 워크로드에서 잘 동작한다.
...
- [Sequential pipeline architecture](/develop/accessing-data/custom-indexer/pipeline-architecture#sequential-pipeline-architecture)
- [Concurrent pipeline architecture](/develop/accessing-data/custom-indexer/pipeline-architecture#concurrent-pipeline-architecture)
...
1. Add dependencies
    ...
    `telemetry_subscribers`에 `Cargo.toml` dependency를 추가한다:
...
1. Run with console enabled
    ...
    **Flag explanations:**
    ...
    - `TOKIO_CONSOLE=1`: `tokio-console`에서 `telemetry_subscribers` 통합을 활성화한다.
...
프레임워크는 수집, pipeline 처리, 데이터베이스 작업, 워터마크 관리 전반에 걸친 광범위한 metrics를 추적한다. 사용 가능한 metrics의 전체 목록과 설명은 [`IndexerMetrics`](https://github.com/MystenLabs/sui/blob/main/crates/sui-indexer-alt-framework/src/metrics.rs)의 `sui-indexer-alt-framework/src/metrics.rs` struct를 참조하라.

주요 metric 범주는 다음을 포함한다:

- **Ingestion metrics:** 전역 checkpoint 및 트랜잭션 처리 통계.
...
기본적으로 metrics는 [Prometheus format](https://prometheus.io/docs/develop/sui-architecture/object-model)의 `0.0.0.0:9184/metrics`에서 제공된다. 다음 주소로 재정의할 수 있다:
