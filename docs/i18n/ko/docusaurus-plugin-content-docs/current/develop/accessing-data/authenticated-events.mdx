...
Authenticated Events는 Sui 스마트 계약의 Move event stream을 암호학적으로 검증 가능하게 제공한다.
...
### Authenticated Events emit하기(Move)
...
event는 event type을 정의한 패키지와 자동으로 연결된다.
해당 stream에 event를 발생시킬 수 있는 것은 그 패키지뿐이다.

### Events와의 backward compatibility

Authenticated Events는 일반 Sui event와 완전히 backwards-compatible하다.
Authenticated event는 검증용 추가 메타데이터를 가진 일반 event일 뿐이므로, 기존 event consumer는 수정 없이 그대로 동작한다.

업그레이드하려면:
1. **Contract change**: Move code에서 `event::emit(...)` 호출을 `event::emit_authenticated(...)`로 바꾼다
2. **Package upgrade**: 업데이트된 패키지를 배포한다(contract upgrade 필요)
...
### Events consume하기(Rust reference client)
...
이는 `stream_id`를 포함한 메타데이터와 함께 일반 Event를 Transaction Effects에 기록한다.

event는 **accumulator settlement** 과정을 통해 checkpoint 경계에서 batch로 정산된다:

1. **Event Emission**: 트랜잭션 실행 중 authenticated event가 발생한다
2. **Consensus Commit Batching**: Authenticated Events는 consensus commit별로 그룹화되고, 각 `stream_id`에 대한 event 위로 merkle tree가 계산된다
3. **Checkpoint Settlement**: 각 checkpoint에서 validator는 다음을 수행하는 settlement 트랜잭션을 실행한다:
   - 각 consensus commit의 merkle root를 stream의 MMR에 append한다
   - 새 MMR 상태와 event 수로 해당 `stream_id`의 `EventStreamHead` 객체를 업데이트한다
...
특정 checkpoint에 EventStreamHead 객체가 존재함을 검증하는 데 사용한다.

**Request Parameters:**
- `object_id` (required): EventStreamHead 객체 ID(패키지 address에서 파생)
- `checkpoint` (required): inclusion을 증명할 checkpoint sequence number

**Response:**
- `object_ref`: 객체 reference(object_id, version, digest)
...
- `object_data`: BCS로 인코딩된 EventStreamHead 객체
...
예를 들어 `n = 13 (1101 in binary with bit 3, 2, and 0 set)`일 때 mountain 크기는 `2^3 = 8`, `2^2 = 4`, `2^0 = 1`이다.
...
4. **Object Inclusion Proof**: EventStreamHead 객체가 특정 checkpoint에서 수정되었음이 증명된다
...
- 패키지당 하나의 stream만 존재한다(stream_id = 패키지 address)
