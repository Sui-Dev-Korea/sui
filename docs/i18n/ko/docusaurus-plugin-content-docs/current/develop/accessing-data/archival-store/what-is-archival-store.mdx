...
- 트랜잭션 history, 객체 lineage, 이전 앱 상태와 같은 UI 기능을 렌더링하기 위해 과거 데이터에 접근한다.
...
- **Archival Service:** 이 스토어에 대한 접근을 노출해 과거 데이터의 포인트 조회를 가능하게 하는 [gRPC](/develop/accessing-data/grpc) `LedgerService` 계층이다.
