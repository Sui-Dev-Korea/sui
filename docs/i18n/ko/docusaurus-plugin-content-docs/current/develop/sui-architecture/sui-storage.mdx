...
모든 블록체인에서의 운영 비용은 두 가지 주요 구성 요소로 이루어진다: **compute**는 로직(트랜잭션)을 처리하는 데 필요한 연산 능력이고, **storage**는 데이터와 트랜잭션 결과를 저장하는 데 필요한 디지털 공간이다.
...
Transaction-per-second (TPS) 비율이 증가에 영향을 미치지만, 전체 트랜잭션과 객체 이력을 저장하는 것이 주요 요인이다.
...
[snapshots](/operators/snapshots)에는 두 가지 유형이 있다:
...
객체를 생성하거나 수정하는 모든 트랜잭션에는 스토리지 수수료가 발생한다.
...
트랜잭션의 총 스토리지 수수료는 트랜잭션의 총 가스 수수료의 일부로 청구된다.

### storage fund

트랜잭션의 총 스토리지 비용은 트랜잭션에서 사용한 각 스토리지 unit을 기준으로 한다.

데이터 1바이트는 100 storage units와 같다.

참고로 1킬로바이트는 100,000 storage units와 같다.

트랜잭션에 1킬로바이트 객체가 포함되면 스토리지 수수료는 760만 MIST 또는 0.0076 SUI가 된다.

이 수수료는 두 부분으로 나뉜다:

1. **Refundable deposit:** 객체가 존재하는 동안 _스토리지 기금_ 에 잠긴다. 객체가 삭제되거나 크기가 줄어들면 이 예치금의 일부가 _스토리지 리베이트_ 로 반환된다.
...
불변 객체는 삭제하거나 수정할 수 없으므로 스토리지 수수료가 영구히 잠기며 리베이트도 받을 수 없다.
