...
실행 이전에 블록을 생성하는 전통적인 블록체인과 달리, Sui는 transaction 실행 이후에 checkpoints를 생성하여 체인 이력에 대한 인증된 기록을 제공한다. Checkpoints에는 이전 checkpoint의 cryptographic hash, checkpoint에 포함된 모든 transaction digests 및 이에 대응하는 transaction effects digests의 목록, 그리고 checkpoint 생성 시점에 위원회를 구성하던 validators의 [quorum](/develop/sui-architecture/consensus#quorum) (3분의 2 초과)으로부터의 서명 집합이 포함된다.
