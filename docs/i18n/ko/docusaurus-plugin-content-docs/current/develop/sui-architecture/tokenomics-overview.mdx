...
각 epoch 동안 users는 Sui 플랫폼에 transactions를 제출하고 validators는 이를 처리한다. 각 transaction에 대해 users는 관련 computation 및 storage gas fees를 지불한다. Users가 이전 transaction 데이터를 삭제하는 경우에는 users가 [storage fees의 일부 리베이트](#storage-fund-reward)를 받는다. Validators는 다른 validators의 동작을 관찰하고 서로의 성능을 평가한다.
