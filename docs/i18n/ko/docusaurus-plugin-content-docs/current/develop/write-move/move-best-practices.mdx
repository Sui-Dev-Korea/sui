...
keywords: ['Move best practices', 'Sui conventions', 'smart contract patterns', 'Move coding standards', 'package organization', 'module design', 'Sui Move development']
---
...
이러한 이유로 `Move.lock` 파일은 항상 패키지에 포함되어야 한다 (`.gitignore` 파일에 추가하지 말아야 한다). 매니페스트 파일의 이전 `published-at` 필드 대신 [자동 주소 관리](/develop/manage-packages/automated-address-management)를 사용한다.
...
모듈은 Move 코드의 주요 구성 요소이다. 모듈은 관련된 기능을 조직화하고 캡슐화하는 데 사용된다. 하나의 객체 또는 데이터 구조체를 중심으로 모듈을 설계한다. 변형 구조체는 복잡성과 버그를 피하기 위해 자체 모듈을 가져야 한다.
...
여기서 public 함수는 상태를 수정하는 함수가고 view 함수는 종종 온체인 게터 또는 오프체인 헬퍼이다. 후자는 객체를 쿼리하여 데이터를 읽을 수 있으므로 필요하지 않다. `init` 함수는 모듈에 존재하는 경우 모듈의 첫 번째 함수여야 한다.

가독성을 높이기 위해 함수는 목적과 사용자 흐름에 따라 정렬하는 것이 좋다. 또한 `admin_set_fees`와 같이 명시적인 함수 이름을 사용하여 함수가 수행하는 작업을 명확하게 할 수 있다.

이상적으로, 테스트 함수는 `[test_only]` 디렉터리에 있는 실제 테스트를 위한 `tests` 헬퍼로만 구성되어야 한다.
...
구조체는 간단한 wrapper, dynamic 필드 key 또는 튜플로 사용할 수 있는 위치 필드를 지원한다.
...
- `new`: 빈 객체를 생성한다.
- `empty`: 빈 구조체를 생성한다.
- `create`: 초기화된 객체 또는 구조체를 생성한다.
- `add`: 값을 추가한다.
- `remove`: 값을 제거한다.
- `exists`: 키가 존재하는지 확인한다.
- `contains`: 컬렉션에 값이 포함되어 있는지 확인한다.
- `borrow`: 구조체 또는 객체의 불변 참조를 반환한다.
- `borrow_mut`: 구조체 또는 객체의 가변 참조를 반환한다.
- `property_name`: 필드의 불변 참조 또는 복사본을 반환한다.
- `property_name_mut`: 필드의 가변 참조를 반환한다.
- `drop`: 구조체를 드롭한다.
- `destroy`: **drop** ability가 있는 값을 가진 객체 또는 데이터 구조체를 파괴한다.
- `destroy_empty`:  **drop** ability가 있는 값을 가진 빈 객체 또는 데이터 구조체를 파괴한다
...
다음 섹션에서는 객체 소유권 모델 및 함수 설계 원칙을 포함하여 Sui에서 Move 개발에 특화된 일반적인 패턴과 모범 사례를 다룬다.

### 공유 객체 사용

Object를 공유하는 라이브러리 모듈은 두 가지 함수를 제공해야 한다: 객체를 인스턴스화하고 반환하는 함수와 이를 공유하는 함수. 이를 통해 호출자는 이를 다른 함수에 전달하고 공유하기 전에 사용자 정의 기능을 실행할 수 있다.
...
프론트엔드에서 트랜잭션 가독성을 높이기 위해 정확한 양의 `Coin` 객체를 값으로 직접 전달한다.
...
구성 가능성을 유지하기 위해 접근 제어에는 address 배열 대신 capability 객체를 사용한다.
...
### 소유 객체와 공유 객체의 data storage

DApp 데이터가 일대일 관계를 갖는 경우 소유 객체를 사용하는 것이 가장 좋다.
