...
description: Sui는 여러 signature scheme을 통해 Move 스마트 계약 내부 검증을 지원한다. 지원되는 signature scheme에는 Ed25519, Secp256k1 recoverable, Secp256k1 non-recoverable, Secp256r1 non-recoverable, Secp256r1 recoverable, BLS G1, BLS G2가 포함된다.
...
transaction authorization에 유효한 서명 방식은 [서명](/develop/transactions/transaction-auth/auth-overview) 섹션을 참조한다.
...
#### Step 2: Move에서 verify method를 호출한다. 모든 입력은 hex format의 bytes로 표현된다:
