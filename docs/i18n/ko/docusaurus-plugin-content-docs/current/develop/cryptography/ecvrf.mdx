...
description: Elliptic curve verifiable random 함수는 난수를 생성하고 그 수가 secret key를 사용해 생성되었음을 증명할 수 있게 하는 cryptographic algorithm이다.
keywords: [ elliptic curve, elliptic, verifiable, verifiable randomness, VRF, verifiable random function ]
---


verifiable random 함수(VRF)은 난수를 생성하고 그 수가 secret key를 사용해 생성되었음을 증명할 수 있게 하는 cryptographic primitive이다.
...
Sui Move framework의 `sui::ecvrf::ecvrf_verify`를 사용하면 스마트 계약 안에서 proof와 output을 검증할 수 있다:
