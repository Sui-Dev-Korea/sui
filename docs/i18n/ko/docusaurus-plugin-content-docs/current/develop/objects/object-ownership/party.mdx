...
object에 대해 [custom transfer policy](/develop/objects/transfers/custom-rules) 를 정의하는 경우 `sui::transfer::party_transfer` 함수를 사용한다. Object가 `store` capability를 가지고 있다면 `sui::transfer::public_party_transfer` 함수를 사용한다.
...
소유 address가 object ID에 해당하는 party object는 [transfer to object](../transfers/transfer-to-object) 메커니즘을 통한 접근이 지원되지 않는다.
