...
Object에 대한 [custom transfer rules](/develop/objects/transfers/custom-rules)를 정의하는 경우 <code>sui::transfer::transfer</code> 함수를 사용해야 한다. Object가 `store` capability를 가진다면 `sui::transfer::public_transfer` 함수를 사용하여 address-owned object를 생성한다.
...
Object의 주소 소유자가 object ID인 경우, [객체로 전송](../transfers/transfer-to-object)에 정의된 메커니즘을 사용하여 transaction 실행 중에 동적 인증 후 접근해야 한다.
