...
[custom transfer policies](/develop/objects/transfers/custom-rules)와 마찬가지로 Sui는 `key`-only object에 대한 custom receivership 규칙의 정의를 허용한다. 특히 `transfer::receive` 함수는 `transfer::receive`가 호출되는 모듈에 정의된 object에만 사용할 수 있으며, 이는 `transfer::transfer` 함수 사용 규칙과 동일하다.
