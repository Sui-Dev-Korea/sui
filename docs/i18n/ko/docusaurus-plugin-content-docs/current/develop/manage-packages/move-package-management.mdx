...
`sui client test-publish --pubfile-path <pubfile> --build-env <env>` 명령은 `<env>` environment에 맞춰 root package를 build하고([build environment 구성](#build-environment-구성) 참조), 그다음 ephemeral publication file `<pubfile>`의 address를 사용해 chain에 게시한다.
...
ephemeral publication file format은 main package와 함께 dependency를 publish하기 쉽게 설계되어 있다. 이를 수행하는 방법은 manual 방식과 automatic 방식 두 가지가 있다.
...
`a`를 publish하면 `packages/Pub.localnet.toml` file이 `a`의 Localnet address entry로 업데이트된다. 그런 다음 `b`를 publish하면 `packages/Pub.localnet.toml`에서 `a`의 address를 가져오고 `b`의 Localnet address도 기록한다.

`--publish-unpublished-deps` command-line argument를 사용하여 이 process를 automate할 수도 있다. 같은 예에서 다음을 실행할 수 있다:
...
System은 ephemeral publication file의 entry를 `source` field로 식별한다. 이러한 field는 pinned dependency이므로 manifest file에 작성된 dependency와 다를 수 있지만, lockfile의 `source` field와 일치해야 한다.
