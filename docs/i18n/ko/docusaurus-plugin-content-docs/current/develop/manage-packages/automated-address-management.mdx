...
[upgrading](/develop/publish-upgrade-packages/upgrade#업그레이드-요구-사항) 시, 패키지의 `UpgradeCap` ID가 필요하다. Automated address management는 `UpgradeCap`을 추적하지 않는다.

또한 패키지의 `[addresses]` 값을 `Move.toml`에서 `0x0`으로 설정하고, 업그레이드 후 `ORIGINAL-ADDRESS`로 복원해야 한다.
...
- **자신의 패키지의 경우:**
    ...
    - 또는, 위의 [adoption steps](#automated-address-management-도입)를 따른다.
...
- 키-값 쌍은 [adopting automated address management](#automated-address-management-도입)에서 설명된 데이터에 대응한다. 이러한 값을 수정하기 위한 진입점으로 `sui move manage-package` 명령을 사용한다.
:::
