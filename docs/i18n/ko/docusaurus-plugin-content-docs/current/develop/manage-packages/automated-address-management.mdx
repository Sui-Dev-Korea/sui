...
[upgrading](/develop/publish-upgrade-packages/upgrade#upgrade-requirements) 시, 패키지의 `UpgradeCap` ID가 필요하다. Automated address management는 `UpgradeCap`을 추적하지 않는다.
...
- 키-값 쌍은 [adopting automated address management](#adopting-automated-address-management)에서 설명된 데이터에 대응한다. 이러한 값을 수정하기 위한 진입점으로 `sui move manage-package` 명령을 사용한다.
:::
