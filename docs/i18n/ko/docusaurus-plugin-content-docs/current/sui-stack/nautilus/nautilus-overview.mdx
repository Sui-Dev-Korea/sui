...
Nautilus는 Sui의 검증 가능한 오프체인 연산 계층이다. 이는 빌더가 [trusted execution environment (TEE)](https://en.wikipedia.org/wiki/Trusted_execution_environment) 또는 [Marlin Oyster](/sui-stack/nautilus/community-dev-tools)와 같은 TEE marketplace에 민감하거나 리소스 집약적인 작업을 위임하면서도, 스마트 계약 기반 검증을 통해 온체인에서 신뢰를 유지할 수 있게 한다.
...
현재는 자체 관리되는 [AWS Nitro Enclave TEEs](https://aws.amazon.com/ec2/nitro/nitro-enclaves/)와 [Marlin](/sui-stack/nautilus/community-dev-tools)을 통한 Dockerized deployment를 지원한다. Move로 작성된 Sui 스마트 계약을 사용하면 온체인에서 AWS 서명 enclave attestation을 검증할 수 있다. 재현 가능한 빌드 템플릿은 [Github repo](https://github.com/MystenLabs/nautilus)를 참조한다.
...
- AWS Nitro Enclaves와 같은 자체 관리되는 TEE에 오프체인 서버를 배포하거나, [Marlin Oyster](/sui-stack/nautilus/community-dev-tools)를 사용해 Dockerized Nautilus 앱을 배포한다.
...
[Nautilus 설계](/sui-stack/nautilus/nautilus-design)와 [Nautilus 사용하기](/sui-stack/nautilus/using-nautilus)를 참조한다.
