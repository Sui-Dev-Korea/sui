...
Sui는 asset owners에게 높은 보안 보장을 제공하도록 설계되었다. coins와 tokens를 포함한 Sui의 자산은 [_objects_](/guides/developer/objects/object-model.mdx)의 타입이며, smart contract의 사전 정의된 로직에 따라 달리 정의되지 않는 한 소유자만 사용할 수 있다. 모든 smart contract는 audit할 수 있으며, 일부 [validators](/concepts/sui-architecture/consensus.mdx)가 프로토콜을 올바르게 따르지 않더라도 네트워크는 이를 올바르게 처리할 수 있다. 이를 _fault tolerance_ 라고 한다.

Sui의 보안 기능은 objects에 대해 특정 속성을 보장한다:

1. object의 소유자만이 그 object에 대해 동작하는 [transaction](/guides/developer/transactions/txn-overview.mdx)을 승인할 수 있다. 승인은 object owner만 아는 private signature key를 사용해 수행된다.

1. 누구나 [shared](/guides/developer/objects/object-ownership/shared.mdx) objects 또는 [immutable](/guides/developer/objects/object-ownership/immutable.mdx) objects에서 동작할 수 있지만, smart contract가 추가 access control logic을 구현할 수 있다.
...
Sui 네트워크는 [set of validators](/concepts/sui-architecture/consensus.mdx)에 의해 운영되며, 이들은 [트랜잭션](/guides/developer/transactions/txn-overview.mdx)을 처리한다. 이들은 네트워크에 제출되고 처리된 어떤 transactions가 유효하고 [_consensus_](/concepts/sui-architecture/consensus.mdx)를 통해 final되어야 하는지에 대해 합의한다.
...
Move smart contract language는 audit과 verification을 쉽게 할 수 있도록 설계되었다. [Move 개념](/concepts/sui-move-concepts)에 관심이 있을 수 있다.
...
Sui는 validators 집합을 주기적으로 결정하기 위해 delegated proof-of-stake를 사용한다. SUI tokens는 각 epoch에서 lock되고 위임되어 다음 epoch의 validators committee를 결정한다. 최소 amount 이상의 delegated stake를 가진 누구나 [operate a Sui validator](/guides/operator/validator/validator-config.mdx).

Validators는 gas fee 수입을 통해 [rewards](/guides/operator/validator/validator-rewards.mdx)를 자신들을 validators로 지원하기 위해 [stake their Sui](/concepts/tokenomics/staking-unstaking) users에게 제공한다. 신뢰성이 낮은 validators와, 그들에게 stake를 위임한 users는 더 낮은 보상을 받을 수 있다. User stake는 몰수될 수 없다.
