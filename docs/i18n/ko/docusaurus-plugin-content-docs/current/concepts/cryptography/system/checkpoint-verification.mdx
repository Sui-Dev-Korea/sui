...
실행 이전에 블록을 생성하는 전통적인 블록체인과 달리, Sui는 transaction 실행 이후에 checkpoints를 생성하여 체인 이력에 대한 인증된 기록을 제공한다. Checkpoints에는 이전 checkpoint의 cryptographic hash, checkpoint에 포함된 모든 transaction digests 및 이에 대응하는 transaction effects digests의 목록, 그리고 checkpoint 생성 시점에 위원회를 구성하던 validators의 [quorum](/concepts/sui-architecture/consensus#quorums) (3분의 2 초과)으로부터의 서명 집합이 포함된다.

둘 다 [validators](/guides/operator/validator-index.mdx)와 [full nodes](/guides/operator/sui-full-node.mdx)는 네트워크와 동기화를 유지하기 위해 checkpoints를 소비한다.

## Verifying checkpoints

풀 노드와 validators는 checkpoint를 신뢰하기 위해 이를 검증한다. 이 검증은 Sui [validator committee](/guides/operator/validator/validator-rewards.mdx)가 그것을 생성했고 그것이 진본임을 확인한다.
...
2. Checkpoints를 검증함으로써 클라이언트는 위원회의 구성을 알 수 있다. 각 [epoch](/concepts/sui-architecture/epochs.mdx)의 마지막 checkpoint에는 다음 epoch의 validator 위원회와 public keys가 포함된다.
