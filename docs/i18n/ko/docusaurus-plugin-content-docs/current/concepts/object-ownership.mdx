...
Party object는 `0x2::transfer::party_transfer` 또는 `0x2::transfer::public_party_transfer` 함수를 사용하여 전송되는 object이며, 전송 받은 대상인 `Party`만 접근할 수 있다. Party object는 단독으로 소유될 수 있지만, address-owned object와 달리 합의에 의해 버전이 관리된다.
</Card>
<Card title="Shared" href="/concepts/object-ownership/shared">
Shared object는 `0x2::transfer::share_object` 함수를 사용하여 공유되며 모든 사람이 접근할 수 있는 object이다. owned object와 달리, shared object는 네트워크 상의 누구나 접근할 수 있다.
</Card>
<Card title="Wrapped" href="/concepts/object-ownership/wrapped">
Move에서는 `struct` 타입의 필드를 다른 struct에 넣어 데이터 구조를 구성할 수 있다.
</Card>
</Cards>
