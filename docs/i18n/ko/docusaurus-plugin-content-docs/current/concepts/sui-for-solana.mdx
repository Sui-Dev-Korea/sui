...
Sui와 Solana 개발의 가장 큰 차이점은 프로그래밍 언어에 있다. Sui는 [**Move**](/concepts/sui-move-concepts.mdx)를 사용하는 반면 Solana는 일반적으로 앱 개발과 관리를 단순화하는 개발 프레임워크와 함께 사용하는 **Rust**에 의존한다. **Anchor**는 가장 인기 있는 Solana 프레임워크이다.
...
| Contract upgrades | Programs는 배포 시 upgradeable 또는 immutable로 표시할 수 있다. Upgrades는 program binary를 교체하지만 동일한 program ID를 유지한다. | 새 contracts는 이전 것과 [layout-compatible](/guides/developer/packages/upgrade#upgrade-requirements)해야 한다. Shared objects의 버전 관리를 고려해야 한다. |
| Mutate contract state | instruction data를 포함한 transactions를 전송한다. Programs는 instructions를 처리하고 account 데이터를 수정한다. Type-safe 클라이언트 상호작용을 위해 Anchor의 IDL(Interface Definition Language)을 사용할 수 있다. | 런타임 [프로그래머블 트랜잭션 블록 (PTB)](/guides/developer/transactions/ptbs/prog-txn-blocks) 구성을 통해 transaction을 전송한다. |
...
Sui의 모든 ownership 유형은 [객체 소유권 유형](/guides/developer/objects/object-ownership)을 참조하라.
...
PTB에 대한 자세한 내용은 [프로그래머블 트랜잭션 블록 (PTB)](/guides/developer/transactions/ptbs/prog-txn-blocks)를 참조하라.
