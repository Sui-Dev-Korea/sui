...
예를 들어, 모듈 초기화 함수는 패키지 업그레이드 시 다시 실행되지 않는다.<br>초기 패키지를 게시할 때 Move는 게시 이벤트 시점에 패키지에 대해 정의한 `init` 함수를 한 번(그리고 오직 한 번만) 실행한다. 패키지의 이후 버전에 포함할 수 있는 모든 `init` 함수는 무시된다. 자세한 내용은 The Move Book의 [Module Initializer](https://move-book.com/programmability/module-initializer.html)를 참조한다.
...
패키지가 `v1.29.0` 이전의 Sui 버전으로 게시되거나 업그레이드된 경우, [the guide for adopting automated address management](automated-address-management#adopting-automated-address-management-for-published-packages)를 따를 수 있다.<br>또는 추가 단계에 대해서는 위의 `Manual Addresses` 탭을 참조한다.
