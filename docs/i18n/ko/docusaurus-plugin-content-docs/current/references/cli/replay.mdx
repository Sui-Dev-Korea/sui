...
Replay 도구를 사용하면 `--trace` 플래그를 전달하여 Move 디버거에서 사용할 수 있는 transaction 실행 추적을 생성할 수 있다. Move 디버거에 관한 자세한 내용은 [Move Trace Debugger](/references/ide/debugger) 페이지를 참조한다. [gas profiling](/references/cli/trace-analysis)과 같은 것은 다양한 [Sui 트레이스 분석](/references/cli/trace-analysis)으로 수행할 수 있다.

## 사용법

Transaction을 replay하는 주요 명령은 다음과 같다. 사용 가능한 모든 플래그와 인수를 보려면 `sui replay --help`를 실행한다.

```sh
$ sui replay --digest <digest>
```

해당 명령을 실행하면 `<cur_directory>/.replay/<digest>` 플래그를 설정하지 않은 한 도구는 `--output-dir`에 digest에 해당하는 디렉터리를 생성한다. 같은 transaction을 두 번 이상 replay하려 해도 출력 디렉터리는 자동으로 덮어쓰지 않는다. 이 디렉터리를 덮어쓰려면 `--overwrite` 플래그를 포함한다.

이전 명령을 성공적으로 실행하면 콘솔에 다음과 유사한 응답이 표시된다.
...
위 답변은 transaction 효과 정보, gas report, 그리고 각 object의 크기 및 비용에 대한 더 자세한 목록을 제공한다. gas cost에 관한 자세한 내용은 [Sui의 가스](/develop/transaction-payment/gas-in-sui) 페이지를 참조한다.

`--digests-path` 플래그를 사용해 여러 transaction digest가 한 줄에 하나씩 포함된 파일 위치를 지정하면 여러 transaction을 한 번에 replay할 수 있다. 기본적으로 개별 transaction 실행의 성공 또는 실패와 관계없이 전체 batch가 replay된다. transaction 중 하나를 replay할 때 오류가 발생하면 `--terminate-early` 플래그를 사용해 batch 실행을 종료할 수 있다.

`~/.sui/sui_config/replay.toml` 파일에 replay 도구 플래그의 기본값을 지정할 수 있다. 현재 지원되는 유일한 section인 `[flags]` section에 긴 이름에서 처음 대시(`--`)를 제거한 형태로 지정해야 한다. 예를 들어 transaction effects 표시를 비활성화하려면 `show-effects = false`가 아니라 `e = false`를 사용할 수 있다. 전체 config file 예시는 다음과 같다.
