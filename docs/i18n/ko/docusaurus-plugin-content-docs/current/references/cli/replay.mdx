...
Replay 도구를 사용하면 `--trace` 플래그를 전달하여 Move 디버거에서 사용할 수 있는 transaction 실행 추적을 생성할 수 있다. Move 디버거에 대한 자세한 내용은 [Move Trace Debugger](../ide/debugger) 페이지를 참조한다. [gas profiling](./trace-analysis)과 같은 것은 다양한 [Sui 트레이스 분석](./trace-analysis#profile-a-transaction)으로 수행할 수 있다.
...
해당 명령을 실행하면 `--output-dir` 플래그를 설정하지 않은 한 도구는 `<cur_directory>/.replay/<digest>`에 digest에 해당하는 디렉터리를 생성한다. 같은 transaction을 두 번 이상 replay하려 해도 출력 디렉터리는 자동으로 덮어쓰지 않는다. 이 디렉터리를 덮어쓰려면 `--overwrite` 플래그를 포함한다.
...
위 답변은 transaction 효과 정보, gas report, 그리고 각 object의 크기 및 비용에 대한 더 자세한 목록을 제공한다. gas cost에 대한 자세한 내용은 [Sui의 가스](/develop/transaction-payment/gas-in-sui) 페이지를 참조한다.
...
`~/.sui/sui_config/replay.toml` 파일에 replay 도구 플래그의 기본값을 지정할 수 있다. 현재 지원되는 유일한 section인 `[flags]` section에 긴 이름에서 처음 대시(`--`)를 제거한 형태로 지정해야 한다. 예를 들어 transaction effects 표시를 비활성화하려면 `e = false`가 아니라 `show-effects = false`를 사용할 수 있다. 전체 config file 예시는 다음과 같다.
