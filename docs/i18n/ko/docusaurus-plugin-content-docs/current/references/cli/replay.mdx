...
Replay 도구를 사용하면 `--trace` 플래그를 전달하여 Move 디버거에서 사용할 수 있는 transaction 실행 추적을 생성할 수 있다. Move 디버거에 대한 자세한 내용은 [Move Trace Debugger](../ide/debugger.mdx) 페이지를 참조한다. [gas profiling](./trace-analysis.mdx)과 같은 것은 다양한 [Sui 트레이스 분석](./trace-analysis.mdx#profile-a-transaction)으로 수행할 수 있다.
...
위 답변은  transaction 효과 정보, 가스 보고서, 그리고 각 object의 크기 및 비용에 대한 더 자세한 목록을 제공한다. 가스 비용에 대한 자세한 내용은 [가스 수수료](../../concepts/tokenomics/gas-in-sui.mdx) 페이지를 참조한다.
