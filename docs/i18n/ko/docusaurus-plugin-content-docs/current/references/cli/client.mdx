...
transaction 재생 및 디버깅 또는 프로파일링에 transaction을 활용하는 방법에 대한 자세한 내용은 [Sui Replay CLI](./replay.mdx), [Move Trace Debugger](../ide/debugger.mdx), 그리고 [Sui 트레이스 분석](./trace-analysis.mdx) 가이드를 참조바란다.
...
이 예제에서는 `sui move` 명령어를 사용한다. 해당 명령어에 대한 자세한 내용은 [Sui Move CLI](./move.mdx)를 참조바란다:
