...
<ImportContent source="console-output/sui-client-help" mode="snippet" />
...
transaction 재생 및 디버깅 또는 프로파일링에 transaction을 활용하는 방법에 대한 자세한 내용은 [Sui Replay CLI](./replay), [Move Trace Debugger](../ide/debugger), 그리고 [Sui 트레이스 분석](./trace-analysis) 가이드를 참조바란다.
...
이 예제에서는 `sui move` 명령어를 사용한다. 해당 명령어에 대한 자세한 내용은 [Sui Move CLI](./move)를 참조바란다:
...
6. `sui client gas`를 사용하여 활성 주소에 가스 요금 지불에 필요한 가스 코인이 있는지 확인한다.
    ...
    ```
7. 코인이 필요한 경우, `sui client faucet` (메인넷에서는 사용 불가)를 이용한다. 가스 토큰 획득에 대한 자세한 내용은 [Get Sui Token](/getting-started/onboarding/get-coins)를 참조바란다.
    ```sh
    $ sui client faucet
    ```
...
<ImportContent source="console-output/sui-client-call-help" mode="snippet" />
