...
[Replaying a transaction](./replay) 은 가스 사용량, 호출된 함수, 실행된 명령어 등 transaction 실행에 대한 자세한 정보가 포함된 추적 파일이 생성된다. Sui CLI를 사용하여 추적 파일을 분석하고 유용한 정보를 추출하기 바란다.
...
이 명령은 현재 작업 디렉터리에 `gas_profile_{TRACE_FILE_NAME}.json` 형식으로 프로파일을 출력한다. `analyze-trace` 명령에 `--output/-o` 플래그를 사용하여 프로파일의 다른 출력 디렉터리를 지정할 수도 있다. 생성된 프로파일을 표시하려면 [speedscope](https://www.speedscope.app/)를 사용한다.
