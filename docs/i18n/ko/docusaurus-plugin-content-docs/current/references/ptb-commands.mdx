...
다음 섹션은 각 PTB command를 설명한다. 표시된 command 시그니처는 개념적인 Move 표현이며, 이 연산은 항상 표준 Move 함수로 표현할 수 있는 것은 아니다.

#### `TransferObjects`

**형식:** `TransferObjects(ObjectArgs, AddressArg)`
...
**반환값:** 비어 있는 result vector이다.

**Signature:** `(vector<forall T: key + store. T>, address): ()`

#### `SplitCoins`

**형식:** `SplitCoins(CoinArg, AmountArgs)`
...
**형식:** `MakeMoveVec(VecTypeOption, Args)`
...
**반환값:** `vector<T>` 타입의 단일 result이다. element는 `NestedResult`로 개별 접근할 수 없으며, 전체 vector를 사용하거나 Move 코드 안에서 `MoveCall`을 통해 element에 접근해야 한다.

**Signature:** `(T...): vector<T>`

#### `MoveCall`

**형식:** `MoveCall(Package, Module, Function, TypeArgs, Args)`
...
**반환값:** function 시그니처에 따라 개수가 동적으로 정해지는 result이다.

다른 command와 달리 argument 사용 방식과 result 개수는 호출되는 Move 함수에 따라 달라진다.

#### `Publish`

**형식:** `Publish(ModuleBytes, TransitiveDependencies)`
...
**반환값:** 새로 publish된 package에 대한 `sui::package::UpgradeCap` 타입의 단일 result이다.

검증 후에는 각 module의 `init` 함수가 module byte vector와 같은 순서로 호출된다.

#### `Upgrade`

**형식:** `Upgrade(ModuleBytes, TransitiveDependencies, Package, UpgradeTicket)`
...
**반환값:** upgrade의 증명을 제공하는 `sui::package::UpgradeReceipt` 타입의 단일 result이다.

upgrade의 자세한 내용은 [패키지 업그레이드](/develop/publish-upgrade-packages/upgrade)를 참조한다.
