...
- `new_display`
  ...
  registry 안에 nested된 publisher와 sender publisher를 사용해 `Display` type에 대한 빈 `TokenizedAsset<T>`를 생성하고 반환한다. 여기서 `T`는 `Publisher` object 안에 encapsulate되어 있다.
...
tokenized asset type creator가 rule 또는 policy 같은 default requirement set을 따르지 않고도 kiosk asset에 대해 이 operation을 활성화할 수 있게 한다.
