...
- **Summary:** 우리는 임의 규모에서 익명 디지털 코인 결제를 지원하는 최초의 Byzantine Fault Tolerant (BFT) 프로토콜인 Zef를 소개한다. Zef는 FastPay와 동일한 통신 및 보안 모델을 따르며, 비동기적이고 저지연이며 선형 확장성을 갖는다. Zef는 사용자 account에 바인딩된 오프체인 certificate로 표현되는 opaque coin을 도입한다. 결제 시 코인의 액면가를 숨기기 위해 무작위 커밋과 NIZK proof를 사용한다. 생성된 코인은 [Coconut](https://arxiv.org/pdf/1802.07344.pdf)의 블라인드 및 무작위화 가능한 임계 익명 자격 증명을 사용해 연결 불가능하게 만든다. 스토리지 비용을 제어하기 위해 account 비활성화 시 데이터를 안전하게 제거할 수 있도록 설계한다. Rust 기반 오픈 소스 구현을 제공하며, AWS 벤치마크에서 이론적 한계 없는 처리량과 1초 미만의 확정 시간을 입증한다. 이는 기존 블록체인 기반 익명 결제 시스템 대비 지연 시간을 세 자릿수 이상 개선한 결과이며, 처리량에 이론적 한계가 없다.
