...
MDX는 Markdown 콘텐츠 안에서 [JSX](https://mdxjs.com/)를 사용할 수 있게 한다.
...
이 기능들은 기여에 필수는 아니지만, 독자의 경험을 향상한다면 포함할 수 있다.
...
1. 탭 콘텐츠가 들어갈 위치에 `<Tabs>` element를 추가한다. 같은 선택 항목을 가진 다른 `Tabs`가 있다면 선택적 `groupId` property를 포함한다.

1. 콘텐츠의 각 섹션마다 `Tabs` element의 자식으로 `<TabItem>` element를 추가한다. 각 `TabItem`에는 `value`와 `label` property 값이 모두 필요하다. 부모 `Tabs`에서 `groupId`를 사용하는 경우, 같은 `groupId` 값을 쓰는 다른 `Tabs`의 `TabItem` 전반에서 `value`가 일관되게 유지되도록 한다. 이렇게 하면 동일한 유형의 정보를 표시하도록 의도된 `TabItem`이 관련된 모든 `Tabs`에서 활성화된다. 이 동작은 페이지와 세션 전반에 걸쳐 유지된다.
...
:::info
콘텐츠, 여는 태그, 닫는 태그 사이에는 반드시 빈 줄을 추가해야 한다. 그렇지 않으면 component가 올바르게 렌더링되지 않는다.
:::
...
이 접근 방식을 사용하기 전에 문서 구조를 다시 검토한다. 이 기능은 일부 상황에서 유용하지만, source 파일의 comment 노이즈를 늘린다.
...
1. 문서에서 `tag` 값을 이름으로 설정한다.
    build 프로세스가 이 줄을 참조된 code로 대체하므로, code가 나타나야 할 위치에 둔다.
    ```ts
...
- `singleSpace` → 간격 정규화
- `highlight` → 강조할 줄에 있는 고유 단어를 사용해 특정 줄 강조(예: `highlight="variable_name"`)
...
## 접을 수 있는 component: details 및 summary
...
## thumbnail carousel이 있는 YouTube player

여러 video를 표시하려면 `YTCarousel`을 사용한다.
