...
Debugging은 trace 생성으로 활성화되며, trace는 [unit test execution](#unit-test-debugging) 중 또는 온체인 [transaction replay](#debugging-on-chain-transactions) 중에 생성할 수 있다.
...
Move function 안으로 step into 하면 관련된 모든 [features](#move-code-debugging-기능), 즉 값 추적, breakpoint 등이 उपलब्ध한 상태로 Move code debugging이 시작된다.
...
- `Move: Trace Move test execution` command를 실행한다.
  ...
  :::info
  이 command는 내부적으로 `sui` 바이너리를 사용하며, 이 바이너리는 [pre-installed](#설치)되어 있어야 하고, 그 위치는 [Move Analyzer VS Code 확장 프로그램](/references/ide/move#build-test-and-trace)이 찾을 수 있어야 한다.
  :::
...
데이터는 replay tool 출력 디렉터리의 하위 디렉터리에 저장되며, 기본값은 `.replay`지만 위치는 [configurable](../../references/cli/replay#사용법)하다.
...
transaction 전반에서 사용된 모든 package, 즉 사용자 수준 package와 system package 모두에 대해 source-level debugging을 균일하게 활성화하고, 원활한 [debugging experience](#모든-package의-정확한-version)를 위해 정확한 package version을 사용하려고 해야 한다.

항상 가능하지는 않지만, 그런 경우에도 [precise versions of system packages](#system-package의-정확한-version)를 찾으면 제한적 source-level debugging을 활성화할 수 있다.
