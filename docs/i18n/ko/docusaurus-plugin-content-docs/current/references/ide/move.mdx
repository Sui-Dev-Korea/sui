...
이 명령은 열려 있는 Move source file에 대한 `Move.toml` 파일을 찾아 적절한 `sui move` command를 실행하기 위해 터미널을 연다.

trace를 생성하려면 trace 생성이 가능한 `sui` binary가 있어야 한다. 자세한 내용은 [Debugger](/references/ide/debugger#install)를 참조한다.
