...
규칙은 [`TokenPolicy`](./token-policy.mdx)에서 모든 액션에 적용할 수 있는 프로그래머블 제한 사항이다. 이는 closed-loop 시스템에서 특정 비즈니스 로직의 컴플라이언스, 규제, 그리고 집행을 위한 도구이다.
...
3. Rule만 구성을 읽을 수 있다.
4. 규칙은 `TokenPolicy` 소유자의 승인 없이 구성을 수정할 수 없다.
...
규칙은 `TokenPolicy`에 저장된 구성을 읽을 수 있다.
