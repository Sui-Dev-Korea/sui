...
fund를 차입해 trading할 때, 최소 [리스크 비율](./contract-information/risk-ratio.mdx)을 유지해야 한다(총 asset 대비 총 debt의 비율이다). position의 risk ratio가 **Liquidation Risk Ratio** 이하로 떨어지면, 누구나 position을 liquidation할 수 있다.
...
DeepBook Margin은 optimal utilization 지점까지는 rate가 점진적으로 증가하고, 이후에는 급격히 상승하는 [kinked interest rate model](./contract-information/interest-rates.mdx)을 사용한다.
...
DeepBook Margin은 position을 자동으로 닫는 [이익 실현 및 손실 제한](./contract-information/tpsl.mdx)을 지원한다:
