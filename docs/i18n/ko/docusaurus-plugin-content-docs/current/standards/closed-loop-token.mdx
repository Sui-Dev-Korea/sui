...
[Currency Standard](/standards/currency.mdx)는 Sui의 오픈 루프 시스템의 예시이다. 코인은 자유로이 움직이며, [wrappable](/guides/developer/objects/object-ownership/wrapped.mdx)이고, [freely transferable](/guides/developer/objects/transfers/custom-rules.mdx#the-store-ability-and-transfer-rules)이며, 어떤 애플리케이션에도 저장할 수 있다. 가장 적절한 현실 세계의 비유는 현금으로, 이는 거의 규제되지 않으며 자유롭게 사용되고 전달될 수 있다.
...
생성하는 토큰의 전송, 지출, 변환에 대해 어떤 규칙이든 설정할 수 있다. 이 규칙은 [토큰 정책](/standards/closed-loop-token/token-policy.mdx)에서 action별로 지정한다. [규칙](/standards/closed-loop-token/rules.mdx)는 요청 승인 또는 검증 로직을 구현하는 데 사용할 수 있는 커스텀 프로그래머블 제한이다.
...
coin 및 token 메서드 비교는 [Coin/Token API 비교](/standards/closed-loop-token/coin-token-comparison.mdx)을 참조한다.
...
Protected action은 기본적으로 비활성화되어 있지만 [`TokenPolicy`](/standards/closed-loop-token/token-policy.mdx)에서 활성화할 수 있다. 또한 특정 action이 성공하기 위해 만족해야 하는 커스텀 제한인 [규칙](/standards/closed-loop-token/rules.mdx)를 설정할 수 있다.
