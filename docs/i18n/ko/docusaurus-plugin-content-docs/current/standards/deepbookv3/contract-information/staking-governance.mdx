...
Stake required는 사용자가 taker 및 maker 인센티브를 활용하기 위해 pool에 stake해 두어야 하는 DEEP 토큰의 amount이다. 각 DeepBook pool은 독립적인 거버넌스를 가지며, 거버넌스는 매 epoch마다 수행될 수 있다. 거버넌스에 대해 더 알아보려면 [설계](../design.mdx#governance)을 참조한다.
