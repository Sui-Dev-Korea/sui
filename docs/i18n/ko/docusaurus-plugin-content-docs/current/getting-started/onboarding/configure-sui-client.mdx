---
title: Sui 클라이언트 설정
description: Sui 클라이언트 구성은 어떤 네트워크에 연결하고 어떤 주소로 트랜잭션을 전송할지 지정한다.
...
[Sui 주소, 키 생성, 복구 구문에 대해 자세히 알아본다](/getting-started/onboarding/get-address).
...
`client.yaml`에는 서로 다른 Sui 네트워크(Testnet, Mainnet, Devnet, Localnet)에 연결하기 위한 구성과 현재 활성 환경이 들어 있다. 현재 활성 환경은 명시적으로 지정하지 않을 때 CLI가 어떤 네트워크를 사용할지 알려 준다.
...
[Sui 주소 생성](/getting-started/onboarding/get-address)에서 Sui 주소에 대해 더 알아본다.
