...
| Contract upgrades | Programs는 배포 시 upgradeable 또는 immutable로 표시할 수 있다. Upgrades는 program binary를 교체하지만 동일한 program ID를 유지한다. | 새 contracts는 이전 것과 [layout-compatible](/develop/publish-upgrade-packages/upgrade#업그레이드-요구-사항)해야 한다. Shared objects의 버전 관리를 고려해야 한다. |
