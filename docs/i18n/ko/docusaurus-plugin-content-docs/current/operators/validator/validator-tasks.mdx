...
기본값이며 권장되는 `max-concurrent-connections: 0` 설정은 validator 간의 state sync에는 영향을 미치지는 않지만, 모든 풀 노드의 동기화를 제한한다. 알려진 풀 노드가 validator로부터 동기화할 수 있도록 [`sui-node` configuration](./validator-config)을 구성할 수 있다.
