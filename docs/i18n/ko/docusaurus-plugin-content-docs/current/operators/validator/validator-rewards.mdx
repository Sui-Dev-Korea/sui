...
Sui는 delegated proof-of-stake (DPoS) system을 사용하며, validator는 자신의 stake를 위임한 SUI token holder에게서 voting power를 받는다. 각 epoch가 끝나면 validator는 performance와 commission rate에 따라 rewards를 받는다. validator staking pool이 새로 위임된 stake를 받기 때문에 rewards는 자동 복리로 누적된다.
...
## 다른 blockchain과 비교한 Sui staking

Sui의 staking mechanism에는 다른 blockchain과 다른 feature가 포함되어 있다:

- Staking은 self-custodial이다. staked SUI token을 owned object에 보관한다.
- Staking rewards는 liquidity-pool-inspired design 때문에 자동 복리로 누적된다.
- 새 stake는 생성한 다음 epoch부터 validator의 voting power에 기여하기 시작한다. 마찬가지로 인출된 stake는 withdrawal 다음 epoch부터 기여를 멈춘다.

<ImportContent source="staking-pool-reqs" mode="snippet" />

## user staking 및 reward
...
Stake 예치 요청은 staking pool에서 즉시 pending state로 들어간다. Slush 같은 wallet은 계정의 pending stake deposit request를 반영한다.

각 epoch 끝에서 network는 수집된 gas fee와 stake subsidy를 staking rewards로 validator와 staker에게 분배한다. validator가 받는 rewards는 다음에 따라 달라진다:

- **Performance:** validator가 performant하게 운영되지 않으면 다른 validator가 tallying rule을 통해 이를 report할 수 있다. 그러면 network는 해당 epoch의 reported validator staking rewards를 slash한다.

- **Commission rate:** commission rate는 validator가 보유하는 staking rewards의 portion을 결정한다. 예를 들어 validator의 commission rate가 10%라면, 각 epoch마다 해당 validator의 staker가 받는 staking rewards 중 10%가 validator 소유의 새 stake object로 validator에게 간다.

### reward withdrawal

network는 validator rewards를 일반 stake object로 분배하므로 withdrawal은 validator와 staker에게 같은 방식으로 동작한다. validator는 `sui_system` module의 [`request_withdraw_stake` function](https://github.com/MystenLabs/sui/blob/6234ae2cc8137f3a2a34cd0aa1ac4fd5f31260e2/crates/sui-framework/packages/sui-system/sources/sui_system.move#L272)을 호출해 stake를 인출하고 rewards를 받을 수 있다.
...
사용자는 active stake의 일부를 인출할 수 없다. 모든 staked SUI를 한 번에 인출해야 한다. 하지만 SUI를 여러 coin으로 split하여 여러 `StakedSui` object를 사용해 stake할 수는 있다. 그러면 `StakedSUI` object 중 일부만 un-staking하여 validator에서 부분 withdrawal을 수행할 수 있다.
