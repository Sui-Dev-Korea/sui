...
steady-state operation에서는 공개 풀 노드 gRPC endpoint pattern `https://fullnode.<network>.sui.io:443`를 사용한다. 예시는 다음과 같다.
...
Prunable 파이프라인의 경우 리텐션 기간을 기준으로 `first-checkpoint`를 계산하라:

- **30-day retention**: 체크포인트 `current_checkpoint - 10368000`부터 시작
- **90-day retention**: 체크포인트 `current_checkpoint - 31104000`부터 시작

| **CLI param**            | **설명** |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `<CONFIG_FILE>`      | Indexer 구성 파일 경로이다. |
| `<DATABASE_URL>`     | Postgres 데이터베이스 connection string이다. |
| `<REMOTE_STORE_GCS_BUCKET>` | indexing에 사용할 GCS bucket 이름이다. HTTPS 대신 GCS API를 직접 사용하므로 백필에 권장된다. [Checkpoint Data Sources](/develop/accessing-data/custom-indexer/indexer-data-integration#checkpoint-data-sources)를 참조하라. |
...
`https://checkpoints.mainnet.sui.io`는 최근 30일 체크포인트만 보존한다. Mainnet에서 30일보다 오래된 백필 또는 genesis부터의 백필에는 `--remote-store-gcs mysten-mainnet-checkpoints-use4`를 통해 `gs://mysten-mainnet-checkpoints-use4`를 사용한다.

사용 중인 체크포인트 bucket에 Requester Pays가 활성화되어 있다면 `--remote-store-gcs`를 사용할 때 GCS credential과 billing project를 구성한다. Sui 객체 스토어 구성에서 관련 필드는 `google_service_account`와 `google_project_id`이다. `google_project_id`는 GCS Requester Pays에 필요한 `x-goog-user-project` header로 전송된다.

Requester Pays에 대한 자세한 내용은 [Use Requester Pays](https://cloud.google.com/storage/docs/using-requester-pays)를 참조한다. Source selection과 steady-state operation guidance는 [Running a Remote Store](/operators/data-management/remote-store-setup) 및 [Archival Stack Setup](/operators/data-management/archival-stack-setup)을 참조한다. 일반적으로 풀 노드 gRPC 엔드포인트를 사용할 수 있다면 `https://checkpoints.mainnet.sui.io`를 steady state의 기본 source로 삼지 않는 것이 좋다.

:::

### 실행 구성 권장 사항 {#indexer-run-config}

아래 TOML 파일을 사용하라. 파이프라인 속도별로 묶어 두었으며, 하나의 instance 안에서는 가장 느린 파이프라인이 전체를 제한하므로 각 파일은 대략 비슷한 속도로 실행되는 파이프라인만 담고 있다.

:::info
백필 시에는 `ingest-concurrency`를 더 높은 값(예: 200)으로 설정하고, 네트워크 tip에서 정상 운영할 때는 20으로 낮추는 것이 좋다.
파이프라인을 backfill할 때는 `ingest-concurrency`를 `{ kind = "adaptive", initial = 200, min = 50, max = 2000 }`처럼 더 높은 초기 concurrency로 설정하는 것을 고려한다. Indexer는 현재 load와 performance를 기준으로 concurrency를 자동 조정한다.
:::
...
<table>
	...
	<tbody>
		...
		<tr>
			...
			<td>**Unprunable**</td>
			<td>완전한 객체 version 대 체크포인트 매핑을 담는 객체 versions 테이블.</td>
			<td>`obj_versions`</td>
			<td>10-14일</td>
		...
		<tr>
			<td>`unpruned.toml`</td>
			<td>**Unprunable**</td>
			<td>다른 쿼리가 의존하는 기반 reference 데이터.</td>
			<td>
				<ul className="pl-4">
...
| `<REMOTE_STORE_URL>`        | indexing에 사용할 체크포인트 bucket URL이다. 가능한 여러 [데이터 sources](/develop/accessing-data/custom-indexer/custom-indexers#data-sources) 중 하나를 사용한다. |
...
`https://checkpoints.mainnet.sui.io`는 최근 30일 checkpoint만 보존한다. Mainnet에서 30일보다 오래된 backfill에는 `--remote-store-url` 대신 indexing framework의 `--remote-store-gcs mysten-mainnet-checkpoints-use4` 옵션으로 `gs://mysten-mainnet-checkpoints-use4`를 사용한다.
...
| `<DATABASE_PATH>`    | RocksDB 데이터베이스 경로이다. |
| `<REMOTE_STORE_URL>` | indexing에 사용할 체크포인트 bucket URL이며, 가능한 여러 [데이터 sources](/develop/accessing-data/custom-indexer/custom-indexers#data-sources) 중 하나이다. 공개 HTTPS checkpoint endpoint는 최근 30일 checkpoint만 보존한다. |
...
Mainnet 전체 이력 backfill에는 `--remote-store-url` 대신 `--remote-store-gcs mysten-mainnet-checkpoints-use4`로 `gs://mysten-mainnet-checkpoints-use4`를 사용한다.
...
<table>
	...
	<tbody>
		<tr>
			...
			<td>Consistent Store API 구성과 이벤트 테이블.</td>
			<td>
				<ul className="pl-4">
					<li>`balances`</li>
...
GraphQL RPC server는 General-purpose Indexer의 데이터베이스(Postgres), Consistent Store, 그리고 [archival 서비스](/develop/accessing-data/archival-store)에서 데이터를 읽는다.

<Tabs className="tabsHeadingCentered--small">
<TabItem value="prereq" label="Prerequisites">

- [x] GraphQL RPC server를 시작하기 전에 **모든 unprunable Indexer 파이프라인**(`obj_versions.toml`과 `unpruned.toml`)이 네트워크 tip까지 완전히 따라잡았는지 확인하라. 이 파이프라인들이 완료되어야만 GraphQL 서비스가 정상적으로 동작한다.
...
| `INDEXER_CONFIG_FILE`            | General-purpose Indexer 구성 파일 경로이다. 서로 다른 파이프라인에 대해 여러 번 지정할 수 있다 |
| `LEDGER_GRPC_URL`                | 아카이브 서비스의 `LedgerService` gRPC API URL |
| `CONSISTENT_STORE_URL`           | [Consistent store](#consistent-store-setup) API URL |
| `DATABASE_URL`                   | Postgres 데이터베이스 connection string |
...
AlloyDB Omni는 vCPU당 8GB RAM을 권장한다 [link](https://docs.cloud.google.com/alloydb/omni/containers/current/docs/plan-installation#hardware). 이보다 적게 할당하면 부하 테스트 중 데이터베이스가 Indexer와 GraphQL 연결을 종료한다.
...
1. 새 파이프라인은 별도의 Indexer instance에서 시작하는 것을 권장한다. 필요하다면 Genesis 이후의 체크포인트에서 시작하도록 `--first-checkpoint <checkpoint>` 플래그를 설정할 수 있다.
   * 참고: `--first-checkpoint`는 해당 파이프라인의 워터마크 record가 없을 때만 적용된다(즉, 파이프라인이 이전에 실행된 적이 없어야 한다). 다른 `--first-checkpoint` 값을 사용하려면 워터마크 record를 수동으로 제거해야 하며, 그렇지 않으면 무시된다.
...
|---------------|-----------|---------------------------------------|-------------|---------------------------------------------------------|
| `VACUUM FULL` | Built-in  | 전체 작업 동안 유지됨                      | 아니오          | [PostgreSQL VACUUM documentation](https://www.postgresql.org/docs/current/sql-vacuum.html) |
| pg_repack     | Extension | 초기 단계와 마지막 단계에서 잠시 발생 | 아니오          | [pg_repack documentation](https://reorg.github.io/pg_repack/)                      |
| pg_squeeze    | Extension | 마지막 단계에서 잠시 발생             | 예          | [pg_squeeze on GitHub](https://github.com/cybertec-postgresql/pg_squeeze)       |
