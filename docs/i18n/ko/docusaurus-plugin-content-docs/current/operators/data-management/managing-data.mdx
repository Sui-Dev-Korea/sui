...
Sui 풀 노드의 data를 관리하는 일은 건전한 Sui network를 유지하는 데 중요하다. 이 주제는 풀 노드 구성을 최적화하는 데 사용할 수 있는 Sui 풀 노드의 data management를 상위 수준에서 설명한다. pruning policies와 archival settings 같은 Sui 풀 노드 관련 자세한 내용은 [Sui 풀 노드 구성](/operators/full-node/sui-full-node)를 참고한다.
...
처음부터 시작하는 풀 노드는 아카이브를 저장하는 cloud bucket을 가리키도록 `fullnode.yaml` configuration file에서 [configuring archival fallback](/operators/data-management/archives#set-up-archival-fallback)을 설정해, 주어진 archive에서 Sui genesis 이후 발생한 transactions를 replay하고(따라서 다시 검증하고) 시작할 수 있다.
...
transactions를 pruning하더라도 archival nodes는 뒤처진 peer nodes가 어떤 정보도 잃지 않도록 도울 수 있다. 자세한 내용은 [Sui 아카이브](/operators/data-management/archives)를 참고한다.
...
### indexing이 있지만 object history가 제한된 full node
...
### 전체 object history가 있지만 transaction history가 pruned된 full node
