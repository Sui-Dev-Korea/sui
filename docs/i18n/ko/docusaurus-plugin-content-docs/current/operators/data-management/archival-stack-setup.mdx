...
추가적인 고급 옵션(channel size, fanout concurrency, backpressure threshold, ingestion setting 등)도 제공되지만, 드물게만 튜닝이 필요하다. 전체 참조는 [Pipeline architecture — Performance tuning](/develop/accessing-data/custom-indexer/pipeline-architecture#performance-tuning)을 참조하라.
