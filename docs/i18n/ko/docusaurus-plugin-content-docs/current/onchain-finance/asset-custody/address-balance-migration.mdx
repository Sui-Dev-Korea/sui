...
## address balance에서 가스 지불하기

### address balance 가스 결제 활성화 여부 확인

address balance gas payment를 사용하기 전에 protocol configuration flag를 확인하여 network에서 feature가 활성화되어 있는지 검증할 수 있다:
...
#### 이후: address balances를 사용하는 sponsored gas
...
// 옵션 A: transaction과 signature를 sponsor에게 보내면 sponsor가 즉시 서명하고 실행할 수 있다
// (`send_tx_to_sponsor`는 placeholder이며, SDK에 이런 API는 없다)
await send_tx_to_sponsor(userSignature, bytes);

// 옵션 B: sponsor에게서 signature를 받아 우리가 직접 제출한다. Sponsor의 서명은 비동기로 일어날 수 있다
...
## RPC를 통해 balance 질의하기

### 이전: coin 기반 balance 질의
...
```typescript
// 이전: balance를 가져온다(RPC가 모든 coin object를 합산한다)
...
## checkpoint data에서 balance change 계산하기
...
## 기존 coin을 address balance로 변환하기
