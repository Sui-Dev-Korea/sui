...
`depositDuringInitialization`을 사용해 margin manager 생성 중, 공유되기 전에 자금을 deposit한다. 이는 `newMarginManagerWithInitializer`와 같은 transaction 안에서, `shareMarginManager`보다 먼저 호출해야 한다. 이 호출은 `Transaction` object를 받는 함수를 반환한다.

**매개변수**

- `manager`: `newMarginManagerWithInitializer`가 반환한 margin manager를 나타내는 `TransactionArgument`이다.
- `poolKey`: DeepBook pool을 식별하는 String이다.
- `coinType`: deposit할 coin type을 식별하는 String이다(예: `'SUI'`, `'DBUSDC'`, `'DEEP'`).
- `amount`: deposit할 amount를 나타내는 Number이다(`amount` 또는 `coin` 중 하나만 제공한다).
- `coin`: deposit할 coin object를 나타내는 `TransactionArgument`이다(`amount` 또는 `coin` 중 하나만 제공한다).
...
risk ratio, assets, debts, Pyth price data를 포함한 margin manager의 종합 state 정보를 쿼리한다.
...
Margin manager가 보유한 개별 asset balances를 쿼리한다.
