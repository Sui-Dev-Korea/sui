...
description: DeepBook EWMA (Exponentially Weighted Moving Average) 가스 가격 penalty system에 대해 학습한다.
keywords:
  [
    ewma,
    gas price,
...
1. 풀에 대해 EWMA가 활성화되어 있어야 한다.
1. 현재 가스 가격 ≥ mean이어야 한다(평균보다 높아야 한다).
...
- **Maker protection:** 네트워크 혼잡 시 taker가 stale maker 주문을 먼저 체결하는 것을 억제한다.
- **Dynamic adjustment:** 시간이 지나면서 변하는 가스 가격 pattern에 자동으로 적응한다.
- **Fair pricing:** 극단적인 조건(3 standard deviation)에서만 penalty를 적용한다.
- **Statistical rigor:** outlier를 식별하기 위해 잘 알려진 통계 기법을 사용한다.

## 기술 참고 사항

- **Update frequency:** EWMA는 주문이 제출될 때 업데이트된다. 타임스탬프가 다를 때만 집계되며(같은 트랜잭션 내의 여러 주문은 한 번만 집계된다).
...
- **Pool-specific:** 각 풀은 독립적으로 추적되는 자체 EWMA state를 가진다.
