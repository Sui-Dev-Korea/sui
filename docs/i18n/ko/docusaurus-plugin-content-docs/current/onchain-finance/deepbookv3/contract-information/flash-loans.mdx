---
title: 플래시 론
description: 하나의 programmable 트랜잭션 블록 안에서 대출과 상환이 모두 이루어지는 uncollaterized loan인 flash loan을 DeepBookV3에서 사용하는 방법을 설명한다.
...
`Pool`에서 베이스 자산을 빌린다. 이 함수는 hot potato를 반환하여 차입자가 같은 트랜잭션 안에서 자산을 반환하도록 강제한다.
...
`Pool`에서 견적 자산을 빌린다. 이 함수는 hot potato를 반환하여 차입자가 같은 트랜잭션 안에서 자산을 반환하도록 강제한다.
...
flash loan으로 빌린 베이스 자산을 `Pool`에 반환한다. 자산이 반환된 경우에만 `FlashLoan` 객체가 unwrap되며, 그렇지 않으면 트랜잭션이 실패한다.
...
flash loan으로 빌린 견적 자산을 `Pool`에 반환한다. 자산이 반환된 경우에만 `FlashLoan` 객체가 unwrap되며, 그렇지 않으면 트랜잭션이 실패한다.
