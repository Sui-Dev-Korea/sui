---
title: 스테이킹 및 거버넌스
description: DEEP 토큰을 stake하는 방법과 taker 및 민트er 인센티브를 활용하는 방법을 학습한다.
...
DeepBook의 새로운 거버넌스 접근 방식은 사용자가 단일 풀의 세 가지 파라미터를 업데이트할 수 있도록 한다:

- Taker fee rate
- Maker fee rate
- Stake required

Stake required는 사용자가 taker 및 maker 인센티브를 활용하기 위해 풀에 stake해 두어야 하는 DEEP 토큰의 amount이다. 각 DeepBook 풀은 독립적인 거버넌스를 가지며, 거버넌스는 매 epoch마다 수행될 수 있다. 거버넌스에 대해 더 알아보려면 [설계](/onchain-finance/deepbookv3/design#governance)을 참조한다.
...
staking을 위해 `balance_manager`에서 DEEP 토큰을 사용할 수 있어야 한다. 사용자의 stake는 다음 epoch에서 활성화된다. 사용자의 활성 stake가 stake required보다 크면 사용자는 reduced taker fees를 받을 수 있고 해당 epoch 동안 거래 수수료 rebates를 누적할 수 있다.
...
`balance_manager`는 충분한 staked DEEP 토큰을 보유해야 한다. `balance_manager` data는 unstaked amount로 업데이트된다. 잔액은 즉시 `balance_manager`로 이체된다.
