...
거래를 수행하려면 관련 `Pool`에 `BalanceManager`와 `TradeProof`를 전달한다. `Pool`과 달리 `BalanceManager` shared object는 어떤 타입의 토큰도 포함할 수 있으므로, 동일한 `BalanceManager`가 여러 `Pool`에 접근하여 다양한 거래 페어링과 상호작용할 수 있다. 더 알아보려면 [BalanceManager](./balance-manager)를 참조한다.
