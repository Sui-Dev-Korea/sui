---
title: 컨트랙트 정보
description: Sui Mainnet에서의 DeepBookV3 contract address, 지원 coin, pool 정보를 제공한다.
keywords:
  [
    deepbook,
    deepbookv3,
...
이 페이지는 Sui Mainnet의 DeepBookV3 contract address, 지원 coin, pool 정보를 포함한다.

## 컨트랙트 버전

DeepBookV3는 upgradeable contract를 사용한다. Contract가 업그레이드되면 별도 언급이 없는 한 이전 버전은 계속 호환되므로 `DEEPBOOK_PACKAGE_ID`만 업데이트하면 된다. 재배포가 일어나는 경우에는 `DEEPBOOK_PACKAGE_ID`, `REGISTRY_ID`, 그리고 모든 pool ID를 업데이트해야 한다.
...
Taker와 maker fee는 governance proposal에 따라 바뀔 수 있다. governance process를 통해 fee를 어떻게 조정할 수 있는지에 대한 자세한 내용은 [스테이킹 및 거버넌스](./contract-information/staking-governance)를 참조하라.
