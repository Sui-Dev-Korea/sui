---
title: 규칙
description: "Rule 모듈은 `TokenPolicy` 내 모든 액션에 적용되는 프로그래머블 제한을 만든다."
keywords:
  - rule module
  - token rules
  - tokenpolicy
...
규칙은 [`TokenPolicy`](/onchain-finance/closed-loop-token/token-policy)에서 모든 액션에 적용할 수 있는 프로그래머블 제한 사항이다. closed-loop 시스템에서 특정 비즈니스 로직의 컴플라이언스, 규제, 집행을 위한 도구이다.

## 규칙 구조

규칙은 `drop` ability를 가진 type으로 표현된다. 이 타입을 witness라고 부른다. 이를 애플리케이션 로직에 encode하거나 더 모듈화된 접근을 위해 별도 모듈에 포함할 수 있다.
...
`TokenPolicy`에서 액션에 [규칙을 추가](/onchain-finance/closed-loop-token/token-policy#adding-rules)한 후에는 해당 액션이 확정을 통과하기 위해 규칙의 stamp가 필요하다.

액션 승인 방법에 관한 자세한 내용은 [액션 승인](/onchain-finance/closed-loop-token/action-request#approving-actions) 섹션을 참조한다.

## 모듈식 규칙

규칙을 별도의 재사용 가능한 모듈로 게시할 수 있다. 이는 서로 다른 토큰 정책에서 사용할 수 있는 규칙 라이브러리를 만들 수 있게 하여, 코드 재사용을 극대화하고 오류 위험을 최소화한다.

Rule 모듈은 `verify`와 유사한 일반 모듈이며, 이 함수는 일반적으로 `TokenPolicy`, [`ActionRequest`](/onchain-finance/closed-loop-token/action-request), `TxContext`를 인수로 받는다. 이 함수는 액션을 검증하고 `ActionRequest`에 규칙 타입을 stamp하는 책임이 있다.
...
`denylist` 또는 `allowlist`와 같은 일부 규칙은 구성이 필요하다. 예를 들어, `denylist`은 특정 액션을 수행할 수 없는 주소 목록을 요구할 수 있다. Rule 모듈은 구성 구조를 정의하고 구성을 추가, 수정, 조회, 제거하기 위한 함수를 제공할 수 있다.
