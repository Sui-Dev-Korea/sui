...
규칙은 [`TokenPolicy`](./token-policy)에서 모든 액션에 적용할 수 있는 프로그래머블 제한 사항이다. 이는 closed-loop 시스템에서 특정 비즈니스 로직의 컴플라이언스, 규제, 그리고 집행을 위한 도구이다.
...
`TokenPolicy`에서 action에 [add a rule](./token-policy#adding-rules)을 추가한 후에는 해당 action이 confirmation을 통과하기 위해 rule의 stamp를 요구한다.

액션을 승인하는 방법에 대한 더 자세한 내용은 [Approving actions](./action-request#approving-actions) 섹션을 참조한다.

## Modular rules

규칙을 별도의 재사용 가능한 모듈로 게시할 수 있다. 이는 서로 다른 token policy에서 사용할 수 있는 rule 라이브러리를 만들 수 있게 하여, 코드 재사용을 극대화하고 오류 위험을 최소화한다.

Rule 모듈은 `verify`와 유사한 일반 모듈이며, 이 함수는 일반적으로 `TokenPolicy`, [`ActionRequest`](./action-request), `TxContext`를 인수로 받는다. 이 함수는 액션을 검증하고 `ActionRequest`에 rule 타입을 stamp하는 책임이 있다.
